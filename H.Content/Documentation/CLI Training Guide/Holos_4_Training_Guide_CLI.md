...
7. Finally, we will export our farm file to a location on our computer. Go to the "**File->Export Farm(s)**" menu to open the "Select farms to export" dialog. In this dialog you will see a list of every farm saved in your Holos installation, and **only the farms whose rows are highlighted will be exported**. Each click toggles a row's selection — click an unselected row to add it to the selection, or click an already-highlighted row to remove it. Click the row for "**My CLI template for export**" to highlight it; if any other farms are already highlighted that you don't want to include, click their rows to deselect them. Confirm that only the farm(s) you want are highlighted, then click "**Export**" and choose where to save the file (multiple highlighted rows will all be written into the same export file). Note the name of the file and the location you have saved the file. To find the full path to a file, you can right-click on the file and get the location. For this demonstration we will use the location of "**C:\CLI\Scenario 1**".
...
> **Important: the files we want to view in this section are the *input* files, which live *outside* of the "Outputs" folder.** The "Outputs" folder contains the result files generated by the CLI run we just completed, not the input files. The input files were created by the CLI alongside (not inside) the "Outputs" folder, directly under our scenario folder (for example, in "C:\CLI\Scenario 1") in subfolders named after the component category (e.g. "Fields", "Beef"). If you find yourself navigating into "Outputs", back out one level — that is the wrong folder for this section.

1. Locate the folder (e.g. "**My CLI template for export**") where we originally saved our exported farm file. This folder sits beside the "**Outputs**" folder, not inside it.
...
4. View the animal component file "**Beef Stockers & Backgrounders**" which was generated by the CLI. This file will be in the "**Beef**" folder. Verify that we have two rows - one for each animal management period. The start weight for each group is in column **M** ("Start weight"). Find the steers row and verify that its column M value is "**250 kg**". Close the file.
...
1. Locate latitude and longitude in the settings file and change each value by 1 degree to give the second farm a different location. For example, if the first farm's latitude is 49.6, change the second farm's latitude to 50.6; if the first farm's longitude is -97.0, change the second farm's longitude to -98.0. Save the settings file once both values have been updated.
