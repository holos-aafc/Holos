...
The same applies **after upgrading Holos**. An input file generated by an earlier version carries that version's carbon values, and the CLI will keep using them, so corrections made to the carbon calculations in a newer release will not reach an existing file on their own. To pick them up, either re-export the farm from the GUI, or clear the columns above for the rows you want recalculated.
