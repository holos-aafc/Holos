...
When there are multiple cuts, the Yield entered should represent the total aboveground biomass yield for all cuts in the year, and the 'Roots returned to soil' and 'Extra-roots returned to soil' values should be divided by the number of cuts to provide an estimate of biomass (and thus C and N) inputs from roots and extra-root material from just a single cut (i.e., an approximation of the final cut) in the year.
...
### 13. How do I interpret the Holos GHG emissions estimates in the Detailed Emissions Report for livestock housed on a field?
For livestock housed on pasture or in a field, enteric CH4 and manure CH4 emissions from those animals are reported under the relevant livestock section in the Detailed Emissions Report. However, direct and indirect N2O emissions from the manure deposited directly on the field by these animals are reported under the relevant field section on the same report, thus the direct and indirect N2O emissions for the relevant months under the livestock section will be zero.
If you wish to view direct and indirect N2O emissions from manure deposited on a field by livestock, you can see a breakdown of these estimates by field, year and emission source in the Multiyear Carbon Modelling report. In this report, you can extract the required information from the following columns:

- N2O-N from Organic Nitrogen (N2O-N_ONdirect) (kg N2O-N field^-1) - This is column DU in the exported .csv report file. The values in this column represent direct N2O-N emissions for the field from all applied manure, i.e., manure deposited directly by grazing cattle plus manure applied from storage. Note: even if the user did not specify any manure applications to the farm's fields, Holos will automatically 'apply' all manure remaining in storage across the farm's fields at the end of the calendar year, therefore the values in this column represent emissions from manure deposited by grazing animals + manure applications specified by the model user + leftover manure;
- N2O-N from Organic Nitrogen (Excluding Any Manure Not Applied N2O-N_ONdirect) (kg N2O-N field^-1) - This is column DV in the exported .csv file. The values in this column represent direct N2O-N emissions for the field ONLY from manure deposited directly by grazing cattle plus any manure applications specified by the user. If the user did not specify any manure application to this field in the Holos interface, these emissions estimates relate only to manure from grazing livestock;
- N2O-N from Organic Nitrogen Leaching (N2O-N_ONleach) (kg N2O-N field^-1) - This is column EA in the exported .csv file. This column reports indirect N2O-N emissions from the entire field from N leaching related to field-deposited or applied manure.
As for the direct N2O emissions, this column also considers 'leftover' stored manure applied to the field at the end of the year;
- N2O-N from Organic Nitrogen Leaching (Excluding Any Manure Not Applied N2O-N_ONleach) (kg N2O-N field^-1) - This is column EB in the exported .csv file. This column reports indirect N2O-N emissions from the field from N leaching, related ONLY to manure deposited directly by grazing cattle plus any manure applications specified by the user. If the user did not specify any manure application to this field in the Holos interface, these emissions estimates relate only to manure from grazing livestock;
- N2O-N from Organic Nitrogen Volatilization (N2O-N_ONvolatilization) (kg N2O-N field^-1) - This is column ED in the exported .csv file. This column reports indirect N2O-N emissions from the entire field from N volatilization related to field-deposited or applied manure.
As for the direct N2O emissions, this column also considers 'leftover' stored manure applied to the field at the end of the year;
- N2O-N from Organic Nitrogen Volatilization (Excluding Any Manure Not Applied N2O-N_ONvolatilization) (kg N2O-N field^-1) - This is column EE in the exported .csv file. This column reports indirect N2O-N emissions from the field from N volatilization, related ONLY to manure deposited directly by grazing cattle plus any manure applications specified by the user. If the user did not specify any manure application to this field in the Holos interface, these emissions estimates relate only to manure from grazing livestock.
