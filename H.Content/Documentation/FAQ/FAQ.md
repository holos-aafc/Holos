...
>*Please note: if the user chooses to override the Holos-estimated default yield values for grazed pastures by specifying a custom yield value on the Details screen, this custom yield should represent the amount of biomass consumed by the grazing livestock in the given year and not the total abovegound biomass produced, as behind the scenes Holos calculates the 'actual' aboveground biomass based on the entered yield value and the specified pasture utilization rate.*
