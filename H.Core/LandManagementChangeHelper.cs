...
namespace H.Core
{
    ...
    {
        public TillagePracticeChangeType GetTillagePracticeChangeType(TillageType currentTillageType,
            TillageType pastTillageType)
        {
            ...
            if (pastTillageType == TillageType.Intensive && currentTillageType == TillageType.NoTill)
                return TillagePracticeChangeType.IntenseToNone;

            if (pastTillageType == TillageType.Reduced && currentTillageType == TillageType.NoTill)
                return TillagePracticeChangeType.ReducedToNone;

            if (pastTillageType == TillageType.Reduced && currentTillageType == TillageType.Intensive)
                return TillagePracticeChangeType.ReducedToIntense;

            if (pastTillageType == TillageType.NoTill && currentTillageType == TillageType.Reduced)
        ...
        /// <summary>
        ///     Methodology taken from version 3 file: 'LandManagementDefaults.vb' line #118 - 125. Version 3 only considered the
        ///     total area of fallow land (past/current) when
        ...
        ///     Column O 'LDF' in 'SoilCarbonDefaults.csv' corresponds to lumCMax values in table 2 for fallow to continuous
        ///     cropping and column Q corresponds to lumCMax values in table 2 for
        ...
        {
            if (currentFallowArea < pastFallowArea) return FallowPracticeChangeType.FallowCroppingToContinous;
        ...
        {
            if (currentArea < pastArea) return PerennialCroppingChangeType.DecreaseInPerennialCroppingArea;
