...
namespace H.Core.Services.Animals
{
    ...
    public class ManureService : IManureService
    {
        ...
        private readonly ManureHandlingSystemProvider
            _manureHandlingSystemProvider = new ManureHandlingSystemProvider();
        ...
        private readonly List<ManureLocationSourceType> _validManureLocationSourceTypes =
            new List<ManureLocationSourceType>
            {
        ...
        public double GetFractionOfTotalManureUsedFromLandApplication(CropViewItem cropViewItem,
            ManureApplicationViewItem manureApplicationViewItem)
        {
            var totalVolumeOfManureCreated = GetTotalVolumeCreated(manureApplicationViewItem.DateOfApplication.Year,
                manureApplicationViewItem.AnimalType);
            var totalVolumeFromApplication =
                manureApplicationViewItem.AmountOfManureAppliedPerHectare * cropViewItem.Area;

            if (totalVolumeOfManureCreated == 0) return 0;
        ...
        public double GetFractionOfTotalManureUsedFromExports(double amountExported, AnimalType animalType, int year)
        {
            ...
            var totalVolumeOfManureCreated = GetTotalVolumeCreated(year, animalType);
            if (totalVolumeOfManureCreated <= 0) return 0;
        ...
        public double GetAmountOfTanUsedDuringLandApplication(CropViewItem cropViewItem,
            ManureApplicationViewItem manualApplicationViewItem)
        {
            var fractionUsed = GetFractionOfTotalManureUsedFromLandApplication(cropViewItem, manualApplicationViewItem);
            var totalTANCreated = GetTotalTANCreated(manualApplicationViewItem.DateOfApplication.Year,
                manualApplicationViewItem.AnimalType);
        ...
        public double GetAmountOfTanUsedDuringLandApplications(CropViewItem cropViewItem)
        {
            ...
            foreach (var manureApplicationViewItem in cropViewItem.ManureApplicationViewItems)
                result += GetAmountOfTanUsedDuringLandApplication(cropViewItem, manureApplicationViewItem);
        ...
        public double GetTotalTANExportedByAnimalType(
            ...
            int year)
        {
            var tanExportedByFarm = GetTANExportedForFarm(farm, year);
            var tanByAnimalType =
                tanExportedByFarm.SingleOrDefault(x => x.Item2.GetCategory() == animalType.GetCategory());
            if (tanByAnimalType != null) return tanByAnimalType.Item1;
            ...
            int year)
        {
            ...
            var typesOfManureExported = GetManureTypesExported(farm, year);
            foreach (var typeExported in typesOfManureExported)
            {
                ...
                foreach (var manureExportViewItem in exportsByType)
                    tanByType += GetAmountOfTanExported(manureExportViewItem, year);
        ...
        public double GetAmountOfTanExported(ManureExportViewItem manureExportViewItem, int year)
        {
            ...
            var fractionUsed =
                GetFractionOfTotalManureUsedFromExports(manureExportViewItem.Amount, manureExportViewItem.AnimalType,
                    year);
            var totalTanCreated =
                GetTotalTANCreated(manureExportViewItem.DateOfExport.Year, manureExportViewItem.AnimalType);
        ...
        public double GetTotalVolumeOfManureExported(int year, Farm farm, AnimalType animalType)
        {
            ...
            total = farm.ManureExportViewItems.Where(x => x.DateOfExport.Year == year && x.AnimalType == animalType)
                .Sum(y => y.Amount);
        ...
        public List<AnimalType> GetManureTypesImported(Farm farm, int year)
        {
            ...
            foreach (var manureApplicationViewItem in cropViewItem.ManureApplicationViewItems.Where(x =>
                         x.IsImportedManure() && x.DateOfApplication.Year == year))
            {
                var category = manureApplicationViewItem.AnimalType.GetCategory();
                if (animalTypes.Contains(category) == false) animalTypes.Add(category);
            }
            ...
            AnimalType animalType)
        {
            ...
            {
                var typesForAnimal = _manureHandlingSystemProvider.GetValidManureStateTypesByAnimalCategory(animalType)
                    .Where(x => x != ManureStateType.Pasture);
        ...
        /// <summary>
        ///     When a user adds a manure application, the choices available for the handling system will vary depending in the
        ///     animal type (beef, dairy, etc.)
        /// </summary>
        public void SetValidManureStateTypes(ManureItemBase manureItemBase, Farm farm)
        {
            ...
            var types = GetValidManureStateTypes(farm, manureItemBase.ManureLocationSourceType,
                manureItemBase.AnimalType).Where(x => x != ManureStateType.Pasture);
            ...
            manureItemBase.ManureStateType =
                manureItemBase.ValidManureStateTypesForSelectedTypeOfAnimalManure.FirstOrDefault();
        }

        public void SetValidManureStateTypes(ManureSubstrateViewItem manureSubstrateViewItem, Farm farm)
        {
            // Can't collect manure from a field and apply to another field or export it
            var types = GetValidManureStateTypes(farm, manureSubstrateViewItem.SourceType,
                manureSubstrateViewItem.AnimalType).Where(x => x != ManureStateType.Pasture);
            manureSubstrateViewItem.ValidManureStateTypesForSelectedTypeOfAnimalManure.UpdateItems(types);

            // Set the selected item to the first item in the updated list
            manureSubstrateViewItem.ManureStateType = manureSubstrateViewItem
                .ValidManureStateTypesForSelectedTypeOfAnimalManure.FirstOrDefault();
        }
        ...
        public double GetVolumeAvailableForExport(int year)
        {
            ...
            foreach (var manureTank in tank) amount += manureTank.VolumeRemainingInTank;
        ...
        public double GetTotalVolumeCreated(int year, AnimalType animalType)
        {
            ...
            var tank = _manureTanks.Where(x =>
                x.Year == year && x.AnimalType.GetCategory() == animalType.GetCategory());
            foreach (var manureTank in tank) amount += manureTank.VolumeOfManureAvailableForLandApplication;
        ...
        public double GetTotalTANCreated(int year)
        {
            ...
            var tank = _manureTanks.Where(x => x.Year == year);
            foreach (var manureTank in tank) amount += manureTank.TotalTanAvailableForLandApplication;
        ...
        public double GetTotalNitrogenCreated(int year)
        {
            ...
            var tank = _manureTanks.Where(x => x.Year == year);
            foreach (var manureTank in tank) amount += manureTank.TotalNitrogenAvailableForLandApplication;
        ...
        public double GetTotalCarbonCreated(int year)
        {
            ...
            var tank = _manureTanks.Where(x => x.Year == year);
            foreach (var manureTank in tank) amount += manureTank.TotalAmountOfCarbonInStoredManure;
        ...
        public double GetTotalCarbonRemainingForFarm(Farm farm, int year, CropViewItem viewItem)
        {
            ...
            var totalCarbonCreated = GetTotalCarbonCreated(year);
            var totalCarbonImported = GetTotalCarbonFromImportedManure(farm, year);
            var totalCarbonExported = GetTotalCarbonFromExportedManure(year, farm);
            var totalCarbonApplied = GetTotalCarbonInputsFromLivestockManureApplications(farm, year);
            ...
            if (totalCarbonCreated == 0 && totalCarbonImported > 0) return 0;
        ...
        {
            var totalRemainingForFarm = GetTotalCarbonRemainingForFarm(farm, year, viewItem);
        ...
        public double GetTotalManureCarbonInputsForField(Farm farm, int year, CropViewItem viewItem)
        {
            ...
            var remaining = GetTotalCarbonRemainingForField(farm, year, viewItem);
        ...
        public double GetTotalNitrogenAppliedToAllFields(int year)
        {
            ...
            var tank = _manureTanks.Where(x => x.Year == year);
            foreach (var manureTank in tank) amount += manureTank.NitrogenSumOfAllManureApplicationsMade;
        ...
        public List<Tuple<double, AnimalType>> GetTotalTanAppliedToField(int year, CropViewItem cropViewItem)
        {
            ...
            foreach (var animalType in typesOfManureAppliedToThisField)
            {
                ...
                var manureApplicationsByType =
                    cropViewItem.ManureApplicationViewItems.Where(x => x.AnimalType == animalType);
                foreach (var manureApplicationViewItem in manureApplicationsByType)
                {
                    var totalTANCreated = GetTotalTANCreated(year, manureApplicationViewItem.AnimalType);
                    var fractionUsed =
                        GetFractionOfTotalManureUsedFromLandApplication(cropViewItem, manureApplicationViewItem);
        ...
        public List<Tuple<double, AnimalType>> GetTotalTanAppliedToAllFields(int year, List<CropViewItem> viewItems)
        {
            ...
            {
                var resultsForField = GetTotalTanAppliedToField(year, cropViewItem);
        ...
        {
            var totalAvailableNitrogen = GetTotalNitrogenCreated(year);
            ...
            if (totalAvailableNitrogen == 0 && totalAppliedNitrogen == 0 && importedNitrogenApplied > 0) return 0;
        ...
        {
            var totalAvailableVolume = GetTotalVolumeCreated(year);
            ...
            {
                foreach (var manureApplicationViewItem in cropViewItem.GetLocalSourcedApplications(year))
                    localSourcedVolumeApplied +=
                        manureApplicationViewItem.AmountOfManureAppliedPerHectare * cropViewItem.Area;

                foreach (var manureApplicationViewItem in cropViewItem.GetManureImportsByYear(year))
                    importedVolumeApplied +=
                        manureApplicationViewItem.AmountOfManureAppliedPerHectare * cropViewItem.Area;
            }

            var totalVolumeApplied = localSourcedVolumeApplied;
            var totalExportedVolume = GetTotalVolumeFromExportedManure(year, farm);

            // If all manure used was imported and none from local sources were used or created then there is no remaining N since all imports are used
            if (totalAvailableVolume == 0 && totalVolumeApplied == 0 && importedVolumeApplied > 0) return 0;
        ...
        public double GetTotalNitrogenFromExportedManure(int year, Farm farm)
        {
            ...
            {
                var nitrogen = GetTotalNitrogenFromExportedManure(manureExportViewItem);
        ...
        public double GetTotalNitrogenFromExportedManure(int year, Farm farm, AnimalType animalType)
        {
            ...
            foreach (var manureExportViewItem in farm.ManureExportViewItems.Where(x =>
                         x.AnimalType.GetCategory() == animalType.GetCategory() && x.DateOfExport.Year == year))
            {
        ...
        public double GetTotalNitrogenFromManureImports(int year, Farm farm, AnimalType animalType)
        {
            ...
            {
                var manureImports = cropViewItem.ManureApplicationViewItems.Where(x =>
                    x.IsImportedManure() && x.AnimalType.GetCategory() == animalType.GetCategory() &&
                    x.DateOfApplication.Year == year);
        ...
        public int GetYearHighestVolumeRemaining(AnimalType animalType)
        {
            ...
            var tanksOrderedByAvailableManure = _manureTanks.Where(y => y.AnimalType.GetCategory() == category)
                .OrderByDescending(x => x.VolumeRemainingInTank).ToList();

            if (tanksOrderedByAvailableManure.Any()) return tanksOrderedByAvailableManure.First().Year;
        ...
        public double GetVolumeAvailableForExport(int year, Farm farm, AnimalType animalType)
        {
            ...
            if (tank != null) amount = tank.VolumeRemainingInTank;
        ...
        public void Initialize(Farm farm, List<AnimalComponentEmissionsResults> animalComponentEmissions)
        {
            ...
            var animalTypes = GetManureCategoriesProducedOnFarm(farm);
            var years = farm.GetYearsWithAnimals();
            var stateTypes = GetManureStatesProducedOnFarm(farm);

            foreach (var year in years)
            foreach (var animalType in animalTypes)
            foreach (var manureStateType in stateTypes)
                GetTank(animalType, year, farm, manureStateType);
        }

        public ManureTank GetTank(AnimalType animalType, int year, Farm farm, ManureStateType manureStateType)
        {
            var tank = GetManureTankInternal(animalType, year, manureStateType);
            ResetTank(tank);
            UpdateAmountsUsed(tank, farm, manureStateType);
            ...
            Farm farm)
        {
            ...
            if (field == null || (field.HasLivestockManureApplicationsInYear(viewItem.Year) == false &&
                                  field.HasImportedManureApplicationsInYear(viewItem.Year) == false)) return result;
        ...
        /// <summary>
        ///     Updates the amounts of manure (and amounts of applied nitrogen) whenever a manure application is added, removed, or
        ///     updated.
        /// </summary>
        ...
        private void UpdateAmountsUsed(ManureTank manureTank, Farm farm, ManureStateType manureStateType)
        {
            ...
            foreach (var manureApplicationViewItem in cropViewItem.ManureApplicationViewItems.Get(
                         manureTank.Year,
                         manureStateType,
                         manureTank.AnimalType,
                         false)) // If the manure was imported from off-farm, we don't update/reduce the amounts in the storage tanks
            {
            ...
            ManureStateType manureStateType)
        {
            ...
            var targetGroupEmissions = GetTargetEmissions(animalComponentResults, manureTank.Year, manureStateType);
            foreach (var groupEmissionsByMonth in targetGroupEmissions)
            {
                manureTank.TotalOrganicNitrogenAvailableForLandApplication +=
                    groupEmissionsByMonth.TotalOrganicNitrogenAvailableCreatedInMonth;
                manureTank.TotalTanAvailableForLandApplication +=
                    groupEmissionsByMonth.TotalAmountOfTanInStoredManureAvailableForMonth;
                manureTank.TotalAmountOfCarbonInStoredManure +=
                    groupEmissionsByMonth.TotalAmountOfCarbonInStoredManureAvailableForMonth;

                // Before any nitrogen from any manure applications have been subtracted from the tank, these two values will be the same
                manureTank.TotalNitrogenAvailableForLandApplication +=
                    groupEmissionsByMonth.TotalAmountOfNitrogenInStoredManureAvailableForMonth;
                manureTank.TotalNitrogenAvailableAfterAllLandApplications +=
                    groupEmissionsByMonth.TotalAmountOfNitrogenInStoredManureAvailableForMonth;

                // Before any volume of manure from field applications have been subtracted from the tank, these two values will be the same
                manureTank.VolumeOfManureAvailableForLandApplication +=
                    groupEmissionsByMonth.TotalVolumeOfManureAvailableForLandApplication * 1000;
                manureTank.VolumeRemainingInTank +=
                    groupEmissionsByMonth.TotalVolumeOfManureAvailableForLandApplication * 1000;
            }
        }
        ...
        private void ResetTank(ManureTank manureTank)
        {
            ...
            var tank = GetManureTankInternal(animalType, year, manureType);
            SetStartingStateOfManureTank(tank, resultsForType, manureType);
        }
        ...
        {
            var tank = _manureTanks.SingleOrDefault(x =>
                x.AnimalType.GetCategory() == animalType.GetCategory() && x.Year == year &&
        ...
        /// <summary>
        ///     Returns all <see cref="GroupEmissionsByMonth" /> that are not housing animals on pasture and the result of
        ///     management periods that have the same year as the <see cref="ManureTank" />.
        /// </summary>
            ...
            ManureStateType manureStateType)
        {
            ...
            foreach (var allGroupEmissions in animalComponentEmissionsResult
            ...
            foreach (var groupEmissionsByMonth in allGroupEmissions.GroupEmissionsByMonths)
            {
                ...
                if (managementPeriod.HousingDetails.HousingType.IsPasture() == false &&
                    groupEmissionsByMonth.MonthsAndDaysData.Year == yearOfTank &&
                    managementPeriod.ManureDetails.StateType == manureStateType) result.Add(groupEmissionsByMonth);
            }
