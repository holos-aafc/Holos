...
namespace H.Core.Services.Animals
{
    public class DigestateService : IDigestateService
    {
        ...
        private readonly List<ManureLocationSourceType> _validDigestateLocationSourceTypes =
        ...
        private readonly List<AnimalComponentEmissionsResults> _animalComponentEmissionsResults;
        ...
        {
            if (farm.AnaerobicDigestionComponents.Any() == false) return new List<DigestorDailyOutput>();
        ...
        public double GetTotalManureNitrogenRemainingForFarmAndYear(int year, Farm farm,
            List<DigestorDailyOutput> digestorDailyOutputs, DigestateState state)
        {
            var tankStates = GetDailyTankStates(farm, digestorDailyOutputs, year);
            ...
            switch (state)
            {
                ...
                {
                    amounts = tankStates.Where(x => x.DateCreated.Year == year).OrderBy(x => x.DateCreated)
                        .Select(x => x.NitrogenFromRawDigestate).ToList();
                    result = amounts.Last();
                ...
                {
                    amounts = tankStates.Where(x => x.DateCreated.Year == year).OrderBy(x => x.DateCreated)
                        .Select(x => x.NitrogenFromSolidDigestate).ToList();
                    result = amounts.Last();
                ...
                {
                    amounts = tankStates.Where(x => x.DateCreated.Year == year).OrderBy(x => x.DateCreated)
                        .Select(x => x.NitrogenFromLiquidDigestate).ToList();
                    result = amounts.Last();
            ...
            switch (state)
            {
                case DigestateState.Raw:
                {
                    if (subtractFieldAppliedAmounts)
                        result = tankStates.Where(x => x.DateCreated.Year == year)
                            .OrderBy(x => x.TotalRawDigestateAvailable).Last().DateCreated;
                    else
                        result = tankStates.Where(x => x.DateCreated.Year == year)
                            .OrderBy(x => x.TotalRawDigestateProduced).Last().DateCreated;
                }
                ...
                case DigestateState.LiquidPhase:
                {
                    if (subtractFieldAppliedAmounts)
                        result = tankStates.Where(x => x.DateCreated.Year == year)
                            .OrderBy(x => x.TotalLiquidDigestateAvailable).Last().DateCreated;
                    else
                        result = tankStates.Where(x => x.DateCreated.Year == year)
                            .OrderBy(x => x.TotalLiquidDigestateProduced).Last().DateCreated;
                }
                ...
                default:
                {
                    if (subtractFieldAppliedAmounts)
                        result = tankStates.Where(x => x.DateCreated.Year == year)
                            .OrderBy(x => x.TotalSolidDigestateAvailable).Last().DateCreated;
                    else
                        result = tankStates.Where(x => x.DateCreated.Year == year)
                            .OrderBy(x => x.TotalSolidDigestateProduced).Last().DateCreated;
                }
        ...
        public List<DigestateTank> GetDailyTankStates(Farm farm, List<DigestorDailyOutput> dailyOutputs, int year)
        {
            ...
            if (component == null) return new List<DigestateTank>();

            return GetDailyTankStates(dailyOutputs, farm, component, year);
        }

        public DigestateTank GetTank(Farm farm, DateTime targetDate, List<DigestorDailyOutput> dailyOutputs)
        {
            var year = targetDate.Year;
            var tanks = GetDailyTankStates(farm, dailyOutputs, year);
            ...
            AnaerobicDigestionComponent component, int year)
        {
            ...
            for (var i = 0; i < outputsForYear.Count; i++)
            {
                ...
                CalculateLiquidAmountsAvailable(
                    outputOnCurrentDay,
                    outputDate,
                    farm,
                    i,
                ...
                CalculateSolidAmountsAvailable(
                    outputOnCurrentDay,
                    outputDate,
                    farm,
                    i,
            ...
            DigestateTank tank)
        {
            ...
            if (totalDigestateCreatedOnDay <= 0) return 0;
        ...
        {
            var dailyResults = GetDailyResults(farm);
            if (dailyResults.Any() == false) return 0;

            var dateOfLastOutput = dailyResults.Max(x => x.Date);
            var tank = GetTank(farm, dateOfLastOutput, dailyResults);
            ...
            ManureLocationSourceType sourceLocation)
        {
            ...
            foreach (var digestateApplicationViewItem in cropViewItem.DigestateApplicationViewItems)
                if (digestateApplicationViewItem.DateCreated.Date == dateTime.Date &&
                    digestateApplicationViewItem.DigestateState == state &&
                    digestateApplicationViewItem.ManureLocationSourceType == sourceLocation)
            ...
            ManureLocationSourceType sourceLocation)
        {
            ...
            foreach (var farmFieldSystemComponent in farm.FieldSystemComponents)
            foreach (var cropViewItem in farmFieldSystemComponent.CropViewItems)
            foreach (var digestateApplicationViewItem in cropViewItem.GetDigestateApplicationViewItems(dateTime, state,
                         sourceLocation))
        ...
        public double GetTotalCarbonAppliedToField(CropViewItem cropViewItem, int year)
        {
            ...
            foreach (var digestateApplicationViewItem in
                     cropViewItem.DigestateApplicationViewItems.Where(x => x.DateCreated.Year == year))
                result += digestateApplicationViewItem.AmountOfCarbonAppliedPerHectare * cropViewItem.Area;
        ...
        public double GetTotalDigestateCarbonInputsForField(Farm farm, int year, CropViewItem viewItem)
        {
            ...
            var remaining = GetTotalCarbonRemainingForField(viewItem, viewItem.Year, farm, component);
            var totalProduced = GetTotalCarbonCreatedNotIncludingFieldApplicationRemovals(year, farm);
        ...
        public double GetTotalNitrogenRemainingAtEndOfYearAfterFieldApplications(int year, Farm farm)
        {
            ...
            if (outputsByYear.Any() == false) return 0;
        ...
        ///     (kg C)
        /// </summary>
        public double GetTotalCarbonRemainingForField(CropViewItem cropViewItem, int year, Farm farm,
            AnaerobicDigestionComponent component)
        {
            ...
            if (outputsForYear.Any() == false) return 0;
            ...
            var totalAreaOfFarm = farm.GetTotalAreaOfFarm(false, year);
        ...
        public double GetTotalCarbonForField(
            ...
            AnaerobicDigestionComponent component)
        {
            var totalAppliedToField = GetTotalCarbonAppliedToField(cropViewItem, year);
            var totalCarbonRemainingForField = GetTotalCarbonRemainingForField(cropViewItem, year, farm, component);
            ...
            AnaerobicDigestionComponent component)
        {
            ...
            var totalRawDigestateUsedForFieldApplications = GetTotalAmountOfDigestateAppliedOnDay(outputDate, farm,
                DigestateState.Raw, ManureLocationSourceType.Livestock);
            var totalRawDigestateFromPreviousDay =
                outputNumber == 0 ? 0 : result.ElementAt(outputNumber - 1).TotalRawDigestateAvailable;
            var totalRawProduced = totalRawDigestateOnThisDay + totalRawDigestateFromPreviousDay;
            var totalRawDigestateAvailableAfterFieldApplications =
                totalRawProduced - totalRawDigestateUsedForFieldApplications;

            // Nitrogen from raw digestate
            var totalNitrogenFromRawDigestateOnThisDay =
                outputOnCurrentDay.TotalAmountOfNitrogenFromRawDigestateAvailableForLandApplication;
            var totalNitrogenFromRawDigestateFromPreviousDay =
                outputNumber == 0 ? 0 : result.ElementAt(outputNumber - 1).NitrogenFromRawDigestate;
            var totalNitrogenFromRawDigestateAvailable =
                totalNitrogenFromRawDigestateOnThisDay + totalNitrogenFromRawDigestateFromPreviousDay;

            // Carbon from raw digestate
            var totalCarbonFromRawDigestateOnThisDay =
                outputOnCurrentDay.TotalAmountOfCarbonInRawDigestateAvailableForLandApplication;
            var totalCarbonFromRawDigestateFromPreviousDay =
                outputNumber == 0 ? 0 : result.ElementAt(outputNumber - 1).CarbonFromRawDigestate;
            var totalCarbonFromRawDigestateAvailable =
                totalCarbonFromRawDigestateOnThisDay + totalCarbonFromRawDigestateFromPreviousDay;

            // There should only be raw amounts if there is no separation performed
            if (component.IsLiquidSolidSeparated == false)
            {
                var totalCarbonUsed = GetTotalAmountOfCarbonAppliedOnDay(outputDate, farm, DigestateState.Raw,
                    ManureLocationSourceType.Livestock);
                var totalNitrogenUsed = GetTotalAmountOfNitrogenAppliedOnDay(outputDate, farm, DigestateState.Raw,
                    ManureLocationSourceType.Livestock);
                ...
                tank.NitrogenFromRawDigestateNotConsideringFieldApplicationAmounts =
                    totalNitrogenFromRawDigestateAvailable;
            ...
            AnaerobicDigestionComponent component)
        {
            ...
            var totalLiquidDigestateUsedForFieldApplications = GetTotalAmountOfDigestateAppliedOnDay(outputDate, farm,
                DigestateState.LiquidPhase, ManureLocationSourceType.Livestock);
            var totalLiquidDigestateFromPreviousDay =
                outputNumber == 0 ? 0 : result.ElementAt(outputNumber - 1).TotalLiquidDigestateAvailable;
            var totalLiquidProduced = totalLiquidFractionOnThisDay + totalLiquidDigestateFromPreviousDay;
            var totalLiquidDigestateAvailableAfterFieldApplications =
                totalLiquidProduced - totalLiquidDigestateUsedForFieldApplications;

            // Nitrogen from liquid digestate
            var totalNitrogenFromLiquidDigestateOnThisDay = outputOnCurrentDay
                .TotalAmountOfNitrogenInRawDigestateAvailableForLandApplicationFromLiquidFraction;
            var totalNitrogenFromLiquidDigestateFromPreviousDay = outputNumber == 0
                ? 0
                : result.ElementAt(outputNumber - 1).NitrogenFromLiquidDigestate;
            var totalNitrogenFromLiquidDigestateAvailable = totalNitrogenFromLiquidDigestateOnThisDay +
                                                            totalNitrogenFromLiquidDigestateFromPreviousDay;

            // Carbon from liquid digestate
            var totalCarbonFromLiquidDigestateOnThisDay = outputOnCurrentDay
                .TotalAmountOfCarbonInRawDigestateAvailableForLandApplicationFromLiquidFraction;
            var totalCarbonFromLiquidDigestateFromPreviousDay =
                outputNumber == 0 ? 0 : result.ElementAt(outputNumber - 1).CarbonFromLiquidDigestate;
            var totalCarbonFromLiquidDigestateAvailable = totalCarbonFromLiquidDigestateOnThisDay +
                                                          totalCarbonFromLiquidDigestateFromPreviousDay;

            if (component.IsLiquidSolidSeparated)
            {
                var totalCarbonUsed = GetTotalAmountOfCarbonAppliedOnDay(outputDate, farm, DigestateState.LiquidPhase,
                    ManureLocationSourceType.Livestock);
                var totalNitrogenUsed = GetTotalAmountOfNitrogenAppliedOnDay(outputDate, farm,
                    DigestateState.LiquidPhase, ManureLocationSourceType.Livestock);
                ...
                tank.NitrogenFromLiquidDigestateNotConsideringFieldApplicationAmounts =
                    totalNitrogenFromLiquidDigestateAvailable;
                tank.CarbonFromLiquidDigestate = totalCarbonFromLiquidDigestateAvailable - totalCarbonUsed;
                tank.CarbonFromLiquidDigestateNotConsideringFieldApplicationAmounts =
                    totalCarbonFromLiquidDigestateAvailable;
            }
        }
            ...
            AnaerobicDigestionComponent component)
        {
            ...
            var totalSolidDigestateUsedForFieldApplications = GetTotalAmountOfDigestateAppliedOnDay(outputDate, farm,
                DigestateState.SolidPhase, ManureLocationSourceType.Livestock);
            var totalSolidDigestateFromPreviousDay =
                outputNumber == 0 ? 0 : result.ElementAt(outputNumber - 1).TotalSolidDigestateAvailable;
            var totalSolidProduced = totalSolidFractionOnThisDay + totalSolidDigestateFromPreviousDay;
            var totalSolidDigestateAvailableAfterFieldApplications =
                totalSolidProduced - totalSolidDigestateUsedForFieldApplications;

            //// Nitrogen from solid digestate
            var totalNitrogenFromSolidDigestateOnThisDay = outputOnCurrentDay
            ...
            var totalNitrogenFromSolidDigestateFromPreviousDay =
                outputNumber == 0 ? 0 : result.ElementAt(outputNumber - 1).NitrogenFromSolidDigestate;
            var totalNitrogenFromSolidDigestateAvailable = totalNitrogenFromSolidDigestateOnThisDay +
                                                           totalNitrogenFromSolidDigestateFromPreviousDay;

            //// Carbon from solid digestate
            var totalCarbonFromSolidDigestateOnThisDay = outputOnCurrentDay
                .TotalAmountOfCarbonInRawDigestateAvailableForLandApplicationFromSolidFraction;
            var totalCarbonFromSolidDigestateFromPreviousDay =
                outputNumber == 0 ? 0 : result.ElementAt(outputNumber - 1).CarbonFromSolidDigestate;
            var totalCarbonFromSolidDigestateAvailable =
                totalCarbonFromSolidDigestateOnThisDay + totalCarbonFromSolidDigestateFromPreviousDay;

            if (component.IsLiquidSolidSeparated)
            {
                var totalCarbonUsed = GetTotalAmountOfCarbonAppliedOnDay(outputDate, farm, DigestateState.SolidPhase,
                    ManureLocationSourceType.Livestock);
                var totalNitrogenUsed = GetTotalAmountOfNitrogenAppliedOnDay(outputDate, farm,
                    DigestateState.SolidPhase, ManureLocationSourceType.Livestock);
                ...
                tank.NitrogenFromSolidDigestateNotConsideringFieldApplicationAmounts =
                    totalNitrogenFromSolidDigestateAvailable;
                tank.CarbonFromSolidDigestate = totalCarbonFromSolidDigestateAvailable - totalCarbonUsed;
                tank.CarbonFromSolidDigestateNotConsideringFieldApplicationAmounts =
                    totalCarbonFromSolidDigestateAvailable;
            }
        }
