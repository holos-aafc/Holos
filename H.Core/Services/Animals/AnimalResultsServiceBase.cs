...
namespace H.Core.Services.Animals
{
    public abstract partial class AnimalResultsServiceBase : IAnimalResultsService
    {
        ...
        /// <summary>
        ///     There will be one of these values for each day, in the view model, get the largest value and show this to user so
        ///     that the optimal TDN value is for all days over management period...
        /// </summary>
            ...
            double currentDmiMax)
        {
            ...
            for (var tdn = currentTdn; tdn <= 100 && targetTdnFound == false; tdn += tdnStep)
            {
                ...
                var grossEnergyIntake = CalculateGrossEnergyIntake(
                    netEnergyForMaintenance,
                    netEnergyForActivity,
                    netEnergyForLactation,
                    netEnergyForPregnancy,
                    netEnergyForGain,
                    ratioOfEnergyForMaintenance,
                    ratioOfEnergyForGain,
                    tdn);
            ...
            ManagementPeriod managementPeriod)
        {
            ...
            dailyEmissions.AmmoniaLossFromStorage = CalculateAmmoniaVolatilizationFromStorage(
                dailyEmissions.AmountOfNitrogenExcreted,
                dailyEmissions.AmountOfNitrogenAddedFromBedding,
                dailyEmissions.VolatilizationForStorage,
            ...
            double adjustedEmissionFactorForHousing)
        {
            dailyEmissions.AmmoniaEmissionRateFromHousing = CalculateAmmoniaEmissionRateFromHousing(
                ...
                adjustedEmissionFactorForHousing);

            dailyEmissions.AmmoniaConcentrationInHousing = CalculateAmmoniaConcentrationInHousing(
                dailyEmissions.AmmoniaEmissionRateFromHousing,
                managementPeriod.NumberOfAnimals);

            dailyEmissions.AmmoniaEmissionsFromHousingSystem =
                CoreConstants.ConvertToNH3(dailyEmissions.AmmoniaConcentrationInHousing);

            dailyEmissions.AdjustedNH3NFromHousing =
                CalculateAdjustedAmmoniaFromHousing(dailyEmissions, managementPeriod);
        }

        /// <summary>
        ///     Calculate ammonia emissions from housing/storage for sheep, swine, and other livestock.
        /// </summary>
        public void CalculateIndirectEmissionsFromHousingAndStorage(GroupEmissionsByDay dailyEmissions,
            ManagementPeriod managementPeriod)
        {
            ...
            dailyEmissions.AmmoniaEmissionRateFromHousingAndStorage = CalculateAmmoniaEmissionRateFromHousingAndStorage(
                dailyEmissions.NitrogenExcretionRate,
                dailyEmissions.RateOfNitrogenAddedFromBeddingMaterial,
                dailyEmissions.FractionOfManureVolatilized);

            dailyEmissions.TotalNitrogenLossesFromHousingAndStorage = CalculateTotalNitrogenLossFromHousingAndStorage(
                dailyEmissions.AmmoniaEmissionRateFromHousingAndStorage,
                managementPeriod.NumberOfAnimals);

            dailyEmissions.AmmoniaEmissionsFromHousingAndStorage =
            ...
            dailyEmissions.AdjustedTotalNitrogenEmissionsFromHousingAndStorage =
                CalculateAmmoniaAdjustmentFromHousingAndStorage(
                    dailyEmissions.TotalNitrogenLossesFromHousingAndStorage,
            ...
            ManagementPeriod managementPeriod)
        {
            ...
            dailyEmissions.ManureVolatilizationN2ONEmission = CalculateManureVolatilizationNitrogenEmission(
                dailyEmissions.ManureVolatilizationRate,
                managementPeriod.NumberOfAnimals);
        }
            ...
            ManagementPeriod managementPeriod)
        {
            ...
            dailyEmissions.ManureN2ONLeachingEmission = CalculateManureLeachingNitrogenEmission(
                dailyEmissions.ManureNitrogenLeachingRate,
                managementPeriod.NumberOfAnimals);
        ...
        /// <returns>
        ///     Amount of TAN entering the storage system each day (minus housing and storage NH3-N emissions) in broiler,
        ///     layer and turkey manure (kg TAN day-1), by animal group and manure management system
        /// </returns>
        ...
        private bool HasMovedToNewYear(DateTime currentDate)
        {
            ...
            if (previousDate.Year != currentDate.Year) return true;
        ...
        protected readonly Table_43_Beef_Dairy_Default_Emission_Factors_Provider _defaultEmissionFactorsProvider =
            new Table_43_Beef_Dairy_Default_Emission_Factors_Provider();
        ...
        private static readonly List<Diet> _diets;
        ...
        public virtual AnimalGroupEmissionResults GetResultsForManagementPeriod(AnimalGroup animalGroup,
            ManagementPeriod managementPeriod, AnimalComponentBase animalComponent, Farm farm)
        {
            ...
            foreach (var month in monthlyBreakdownForManagementPeriod)
            {
                ...
                {
                    if (HasMovedToNewYear(currentDate)) previousDaysEmissions = null;
        ...
        public virtual AnimalGroupEmissionResults GetResultsForGroup(AnimalGroup animalGroup, Farm farm,
            ...
            Farm farm)
        {
            ...
            {
                var animalGroupEmissionResult = GetResultsForGroup(animalGroup, farm, animalComponent);
        ...
        public double CalculateDryMatterIntakeForCalves(
            ...
            bool areMilkFedOnly)
        {
            if (areMilkFedOnly) return weight * 0.01;
        ...
        /// <param name="dailyTemperature">
        ///     Daily temperature (upper limit = 20, temperatures above 20ºC or cattle are house in a
        ///     barn, use 20ºC)
        /// </param>
        ...
        {
            if (dailyTemperature > 20) dailyTemperature = 20;
        ...
        public double CalculateAverageDailyWeightGain(double initialWeight,
            ...
            double numberOfDays)
        {
            if (Math.Abs(numberOfDays) < double.Epsilon) return 0;
        ...
        {
            if (Math.Abs(totalDigestibleNutrient) < double.Epsilon) return 0;
        ...
        /// <param name="ratioOfEnergyAvailableForMaintenance">
        ///     Ratio of net energy available in diet for maintenance to digestible
        ///     energy consumed
        /// </param>
        /// <param name="ratioOfEnergyAvailableForGain">
        ///     Ratio of net energy available in diet for gain to digestible energy
        ///     consumed
        /// </param>
        ...
        /// <param name="percentageForageInDiet">
        ///     Percentage of forage in diet (% DM). Will be used to determine the grain content
        ///     of the diet.
        /// </param>
        ...
        {
            var totalGrainInDiet = 100 - percentageForageInDiet;
            if (totalGrainInDiet < 0) totalGrainInDiet = 0;

            var urinaryEnergy = 0.04;
            if (totalGrainInDiet >= 85) urinaryEnergy = 0.02;
        ...
        {
            var averageTemperatureOverLast30Days = CalculateDegreesKelvinOverLast30Days(farm, dailyEmissions);
            dailyEmissions.AverageTemperatureOverLast30Days = averageTemperatureOverLast30Days;

            dailyEmissions.ClimateFactor = CalculateClimateFactor(
                averageTemperatureOverLast30Days);

            dailyEmissions.VolatileSolidsProduced = CalculateVolatileSolidsProduced(
                dailyEmissions.VolatileSolids,
                managementPeriod.NumberOfAnimals);

            dailyEmissions.VolatileSolidsLoaded = CalculateVolatileSolidsLoaded(
                dailyEmissions.VolatileSolidsProduced);

            // (On days that the liquid manure is emptied, there is no carryover so VolatileSolidsAvailable = VolatileSolidsLoaded)
            dailyEmissions.VolatileSolidsAvailable = CalculateVolatileSolidsAvailable(
                dailyEmissions.VolatileSolidsLoaded,
                previousDaysEmissions == null ? 0 : previousDaysEmissions.VolatileSolidsAvailable,
                previousDaysEmissions == null ? 0 : previousDaysEmissions.VolatileSolidsConsumed);

            dailyEmissions.VolatileSolidsConsumed = CalculateVolatileSolidsConsumed(
                dailyEmissions.ClimateFactor,
                dailyEmissions.VolatileSolidsAvailable);

            dailyEmissions.ManureMethaneEmission = CalculateLiquidManureMethane(
                dailyEmissions.VolatileSolidsConsumed,
            ...
            if (managementPeriod.ManureDetails.StateType.IsCoveredSystem())
            {
                ...
                dailyEmissions.ManureMethaneEmission = CalculateLiquidManureMethaneForCoveredSystem(
                    dailyEmissions.ManureMethaneEmission,
                    reductionFactor);
            }
        }
        ...
        public double CalculateDegreesKelvinOverLast30Days(Farm farm, GroupEmissionsByDay groupEmissionsByDay)
        {
            ...
            foreach (var temperature in temperatures)
            {
                ...
                if (degreesCelsius <= 1) degreesCelsius = 1;
        ...
        public void CalculateCarbonInStorage(
            ...
            ManagementPeriod managementPeriod)
        {
            dailyEmissions.AmountOfCarbonLostAsMethaneDuringManagement = CalculateCarbonLostAsMethaneDuringManagement(
                dailyEmissions.ManureMethaneEmission);

            dailyEmissions.AmountOfCarbonInStoredManure = CalculateAmountOfCarbonInStoredManure(
            ...
            dailyEmissions.AccumulatedAmountOfCarbonInStoredManureOnDay = CalculateAmountOfCarbonInStorageOnCurrentDay(
                previousDaysEmissions == null ? 0 : previousDaysEmissions.AccumulatedAmountOfCarbonInStoredManureOnDay,
                dailyEmissions.AmountOfCarbonInStoredManure);
        ...
        /// <param name="dailyAmountOfCarbonLostAsMethaneDuringManagement">
        ///     Carbon lost as methane during manure management (kg C
        ///     day^-1)
        /// </param>
        ...
        {
            var result = dailyFecalCarbonExcretion + dailyAmountOfCarbonFromBedding -
                         dailyAmountOfCarbonLostAsMethaneDuringManagement;
        ...
        ///     Equation 4.1.3-15
        /// </summary>
        /// <param name="amountOfCarbonInStorageInPreviousDay">
        ///     Amount of C in stored manure on the previous day for each animal
        ///     group and manure management system (kg C)
        /// </param>
        /// <param name="amountOfCarbonFlowingIntoStorage">
        ///     Amount of C flowing into storage from manure on the current day for each
        ///     animal group and manure management system (kg C day^-1)
        /// </param>
            ...
            double totalNumberOfYoungAnimalsOnDate)
        {
            ...
            if (animalGroup.GroupType.IsPregnantType())
                // Equation 4.2.1-2
                dailyEmissions.ProteinRetainedForPregnancy = CalculateProteinRetainedForPregnancy();
            ...
            if (isLactatingAnimalGroup)
            {
                ...
                // Equation 4.2.1-3
                dailyEmissions.ProteinRetainedForLactation = CalculateProteinRetainedForLactation(
                    managementPeriod.MilkProduction,
                    managementPeriod.MilkProteinContent,
                    totalNumberOfYoungAnimalsOnDate,
                    managementPeriod.NumberOfAnimals,
                    animalsAreAlwaysLactating);
            }

            if (managementPeriod.HasGrowingAnimals)
            {
                // Equation 4.2.1-4
                dailyEmissions.EmptyBodyWeight = CalculateEmptyBodyWeight(
                    dailyEmissions.AnimalWeight);

                // Equation 4.2.1-5
                dailyEmissions.EmptyBodyGain = CalculateEmptyBodyGain(
                    dailyEmissions.AverageDailyGain);

                // Equation 4.2.1-6
                dailyEmissions.RetainedEnergy = CalculateRetainedEnergy(
                    dailyEmissions.EmptyBodyWeight,
                    dailyEmissions.EmptyBodyGain);

                // Equation 4.2.1-7
                dailyEmissions.ProteinRetainedForGain = CalculateProteinRetainedForGain(
                    dailyEmissions.AverageDailyGain,
                    dailyEmissions.RetainedEnergy);
            }

            // Equation 4.2.1-8
            dailyEmissions.NitrogenExcretionRate = CalculateNitrogenExcretionRate(
                dailyEmissions.ProteinIntake,
                dailyEmissions.ProteinRetainedForPregnancy,
                dailyEmissions.ProteinRetainedForLactation,
            ...
            bool animalsAreAlwaysLactating)
        {
            ...
            if (animalsAreAlwaysLactating) return milkProduction * proteinContentOfMilk;
        ...
        {
            if (Math.Abs(averageDailyGain) < double.Epsilon) return 0;
        ...
        {
            if (crudeProteinInDiet < 0.09) return 0.4;

            if (crudeProteinInDiet >= 0.09 && crudeProteinInDiet < 0.15) return 0.57;
        ...
        /// </summary>
        /// <param name="nitrogenExcretionRate">N excretion rate (kg head^-1 day^-1)</param>
        /// <param name="fractionOfNitrogenExcretedInUrine">
        ///     Fraction of N excreted in urine (urinary-N or urea-N fraction), varied
        ///     with diet CP content (kg N kg^-1 total N excreted)
        /// </param>
        ...
        /// </summary>
        /// <param name="nitrogenExcretionRate">N excretion rate (kg head^-1 day^-1)</param>
        /// <param name="tanExcretionRate">
        ///     Fraction of N excreted in urine (urinary-N or urea-N fraction), varied with diet CP
        ///     content (kg N kg^-1 total N excreted)
        /// </param>
        ...
        /// <param name="amountOfNitrogenAddedFromBedding">
        ///     Total amount of nitrogen added from bedding materials (kg N animal^-1
        ///     day^-1)
        /// </param>
        ...
        /// <param name="rateOfNitrogenAddedFromBedding">
        ///     Total amount of nitrogen added from bedding materials (kg N animal^-1
        ///     day^-1)
        /// </param>
        /// <param name="volatilizationFraction">Volatilization fraction for sheep, swine and other livestock groups</param>
        /// <returns>
        ///     Manure N losses via NH3 volatilization during housing and storage for sheep, swine, and other livestock manure
        ///     systems (kg NH3-N)
        /// </returns>
        ...
        ///     Equation 4.3.4-7
        /// </summary>
        /// <param name="ammoniaEmissionRate">
        ///     Manure N losses via NH3 volatilization during housing and storage for sheep, swine,
        ///     and other livestock manure systems (kg NH3-N)
        /// </param>
        /// <param name="numberOfAnimals">Number of animals</param>
        /// <returns>
        ///     Total manure N losses via NH3 volatilization during housing and storage for sheep, swine, and other livestock
        ///     manure systems (kg NH3-N)
        /// </returns>
        ...
        ///     Equation 4.5.3-10
        /// </summary>
        /// <param name="totalAmmoniaLossFromHousingAndStorage">
        ///     Total manure N losses via NH3 volatilization during housing and
        ///     storage for sheep, swine, and other livestock manure systems (kg N)
        /// </param>
        /// <param name="manureVolatilizationEmissions">
        ///     Manure volatilization N emissions during the housing and manure storage
        ///     stages (kg N2O-N day-1)
        /// </param>
        /// <returns></returns>
        ...
        /// <returns>
        ///     Ambient temperature-based adjustments used to correct default NH3 emission for feedlot (confined no barn)
        ///     housing, or pasture (unitless)
        /// </returns>
            ...
            bool isDairyIndoorHousing = false)
        {
            ...
            if (isDairyIndoorHousing == false) temperatureAdjustment += 2;
        ...
        /// <param name="temperatureAdjustment">
        ///     Ambient temperature-based adjustments used to correct default NH3 emission factor
        ///     (Table 17)
        /// </param>
        ...
        /// <param name="fractionOfTanImmobilizedToOrganicNitrogen">
        ///     Fraction of TAN that is immobilized to organic N during manure
        ...
        /// <param name="fractionOfOrganicNitrogenMineralizedAsTanDuringManureStorage">
        ///     Fraction of organic N that is mineralized as
        ///     TAN during manure storage, dimensionless
        /// </param>
        ...
        ///     Equation 4.3.3-12
        /// </summary>
        /// <param name="tanInStorageOnPreviousDay">
        ///     TAN in storage on the previous day for each animal group and manure management
        ///     system (kg TAN)
        /// </param>
        ...
        public double CalculateAmountOfTanInStorageOnDay(double tanInStorageOnPreviousDay,
            double flowOfTanEnteringStorage)
        {
        ...
        ///     Equation 4.3.3-9
        /// </summary>
        /// <param name="ambientTemperatureAdjustmentStorage">
        ///     Ambient temperature-based adjustments used to correct default NH3
        ///     emission factors for manure storage (compost, stockpile/deep bedding)
        /// </param>
        /// <param name="ammoniaEmissionFactorStorage">
        ///     Default ammonia emission factor for manure stores (deep bedding, solid
        ///     storage/stockpile, compost (passive, active))
        /// </param>
        ...
        /// <param name="volatilizationFraction">
        ///     Fraction of manure N volatilized as NH3 and NOx from the specific manure
        ///     management system
        /// </param>
        ...
        public double CalculateAccumulatedTanAvailableForLandApplication(
            double accumulatedTANEnteringStorageSystemOnDay)
        {
        ...
        /// <summary>
        ///     Equation 4.5.2-3
        ///     Total organic N available for land application considers fecal N and bedding N as an input and the losses due to
        ///     mineralization during NH3 emission and N loss as direct N2O.
        /// </summary>
        /// <param name="fecalNitrogenExcretion">Total nitrogen excreted through feces (kg N animal^-1 day^-1)</param>
        /// <param name="beddingNitrogen">Bedding nitrogen (kg N)</param>
        /// <param name="fractionOfMineralizedNitrogen">
        ///     Fraction of organic N (fecal and bedding N) that is mineralized as TAN
        ///     during manure storage, dimensionless
        /// </param>
        ...
        /// <returns>
        ///     Amount of organic N entering the pool of available manure organic N each day for each animal group and
        ///     management system (kg N day^-1)
        /// </returns>
        ...
        {
            var result =
                fecalNitrogenExcretion + beddingNitrogen -
                ((fecalNitrogenExcretion + beddingNitrogen) * fractionOfMineralizedNitrogen + directManureEmissions +
                 leachingEmissions + nO3NLeachingEmissions);
        ...
        public void CalculateOrganicNitrogen(
            ...
            GroupEmissionsByDay previousDaysEmissions)
        {
            dailyEmissions.OrganicNitrogenCreatedOnDay = CalculateOrganicNitrogenCreatedOnDay(
            ...
            dailyEmissions.AccumulatedOrganicNitrogenAvailableForLandApplicationOnDay =
                CalculateOrganicNitrogenAvailableForLandApplicationOnDay(
        ...
        ///     Equation 4.5.2-6
        /// </summary>
        /// <param name="tanAvailableForLandApplication">
        ///     Total ammonical nitrogen (TAN)/inorganic nitrogen in stored beef manure
        ///     from different housing and storage systems (kg TAN day^-1)
        /// </param>
        /// <param name="organicNitrogenAvailableForLandApplication">
        ///     Total organic nitrogen in stored beef manure from different
        ///     housing and storage systems (kg N day^-1)
        /// </param>
        ...
        {
            if (nitrogenFromManure == 0) return 0;
        ...
        /// <returns>
        ///     Total volume of manure available for land application (1000 kg wet weight for solid manure, 1000 L for liquid
        ///     manure)
        /// </returns>
            ...
            double nitrogenContentOfManure)
        {
            ...
            if (nitrogenContentOfManure <= 0) return 0;
        ...
        /// </summary>
        /// <param name="nitrogenFromManure">Total available manure nitrogen in stored manure (kg N year^-1) </param>
        /// <param name="nitrogenConcentration">
        ///     Nitrogen concentration of manure (default values for different management system;
        ///     kg N per 1000 kg wet weight for solid manure and kg N per 1000 litres for liquid manure
        /// </param>
        /// <returns>
        ///     Total volume of manure produced from different housing and storage systems (1000 kg wet weight for solid
        ///     manure and 1000 litres for liquid manure)
        /// </returns>
        public double CalculateTotalVolumeManure(
            double nitrogenFromManure,
            double nitrogenConcentration)
        {
            if (nitrogenConcentration == 0) return 0;
        ...
        {
            if (areMilkFedOnly) return calfProteinIntakeFromMilk;
        ...
        ///     In v3 the emissions from manure application were separated into emissions from solid manure and emissions from
        ///     liquid manure. In v4 Aklilu updated
        ///     the manure composition information so that specific emissions can be calculated from each different type of manure
        ///     handling system.
        ///     Assumed that all available manure created in the month is applied to a field - must therefore always calculate CO2
        ///     spreading emissions
        /// </summary>
        ...
        ///     Equation 4.3.5-4
        /// </summary>
        /// <param name="dailyAmmoniaEmissionsFromHousing">
        ///     Daily NH3-N emissions during housing of beef cattle (confined-no barn
        ///     (feedlot), barn), dairy cattle (tie-stall barns, free-stall barns, milking parlours, yards/exercise lots and
        ///     pasture), and broilers, layers and turkeys (kg NH3-N day^-1)
        /// </param>
        /// <param name="amountOfNitrogenExcreted">
        ///     Total amount of N excreted by beef or dairy cattle or broilers, layers or
        ///     turkeys (kg N day^-1)
        /// </param>
        ...
        /// </param>
        /// <param name="amountOfNitrogenFromBedding">Total amount of N added from bedding materials (kg N day^-1)</param>
        /// <param name="volatilizationFractionFromHousing">
        ///     Fraction of manure N volatilized as NH3 and NOx during the housing
        ///     stage
        /// </param>
        ...
        ///     Equation 4.3.5-6
        /// </summary>
        /// <param name="ammoniaFromHousing">
        ///     Daily NH3-N emissions during housing of beef cattle (confined-no barn (feedlot),
        ///     barn), dairy cattle (tie-stall barns, free-stall barns, milking parlours, yards/exercise lots and pasture), and
        ///     broilers, layers and turkeys (kg NH3-N day^-1)
        /// </param>
        /// <param name="ammoniaVolatilizedDuringHousing">
        ///     Manure volatilization N emissions during the housing stage (kg N2O-N
        ///     day^-1)
        /// </param>
        ...
        /// </param>
        /// <param name="amountOfNitrogenFromBedding">Total amount of N added from bedding materials (kg N day^-1)</param>
        /// <param name="volatilizationFractionFromStorage">
        ///     Fraction of manure N volatilized as NH3 and NOx during the housing
        ///     stage
        /// </param>
        /// <param name="emissionFactorForVolatilization">Emission factor for volatilization [kg N2O-N (kg N)^-1]</param>
        ...
        ///     Equation 4.3.5-9
        /// </summary>
        /// <param name="ammoniaFromStorage">
        ///     Daily NH3-N emissions during housing of beef cattle (confined-no barn (feedlot),
        ///     barn), dairy cattle (tie-stall barns, free-stall barns, milking parlours, yards/exercise lots and pasture), and
        ///     broilers, layers and turkeys (kg NH3-N day^-1)
        /// </param>
        /// <param name="ammoniaVolatilizedDuringStorage">
        ///     Manure volatilization N emissions during the housing stage (kg N2O-N
        ///     day^-1)
        /// </param>
        /// <returns>Adjusted daily NH3-N emissions from housing (kg NH3-N day-1)</returns>
