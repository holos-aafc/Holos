...
namespace H.Core.Services.Animals
{
    public class AnimalResultsService : IAnimalService
    {
        ...
        private readonly OtherLivestockResultsService
            _otherLivestockResultsService = new OtherLivestockResultsService();
        ...
        public List<AnimalComponentEmissionsResults> GetAnimalResults(Farm farm)
        {
            ...
            results.AddRange(
                _otherLivestockResultsService.CalculateResultsForAnimalComponents(
                    farm.OtherLivestockComponents.Cast<AnimalComponentBase>(), farm));
            results.AddRange(
                _swineResultsService.CalculateResultsForAnimalComponents(
                    farm.SwineComponents.Cast<AnimalComponentBase>(), farm));
            results.AddRange(
                _poultryResultsService.CalculateResultsForAnimalComponents(
                    farm.PoultryComponents.Cast<AnimalComponentBase>(), farm));
            results.AddRange(
                _sheepResultsService.CalculateResultsForAnimalComponents(
                    farm.SheepComponents.Cast<AnimalComponentBase>(), farm));
            results.AddRange(
                _dairyCattleResultsService.CalculateResultsForAnimalComponents(
                    farm.DairyComponents.Cast<AnimalComponentBase>(), farm));
            results.AddRange(
                _beefCattleResultsService.CalculateResultsForAnimalComponents(
                    farm.BeefCattleComponents.Cast<AnimalComponentBase>(), farm));
        ...
        public AnimalGroupEmissionResults GetResultsForGroup(AnimalGroup animalGroup, Farm farm,
        ...
        public AnimalGroupEmissionResults GetResultsForManagementPeriod(AnimalGroup animalGroup, Farm farm,
            AnimalComponentBase animalComponent, ManagementPeriod managementPeriod)
        {
            ...
            if (animalType.GetCategory() == AnimalType.Beef)
                result = _beefCattleResultsService.GetResultsForManagementPeriod(animalGroup, managementPeriod,
                    animalComponent, farm);
            else if (animalType.GetCategory() == AnimalType.OtherLivestock)
                result = _otherLivestockResultsService.GetResultsForManagementPeriod(animalGroup, managementPeriod,
                    animalComponent, farm);
            else if (animalType.GetCategory() == AnimalType.Swine)
                result = _swineResultsService.GetResultsForManagementPeriod(animalGroup, managementPeriod,
                    animalComponent, farm);
            else if (animalType.GetCategory() == AnimalType.Poultry)
                result = _poultryResultsService.GetResultsForManagementPeriod(animalGroup, managementPeriod,
                    animalComponent, farm);
            else if (animalType.GetCategory() == AnimalType.Sheep)
                result = _sheepResultsService.GetResultsForManagementPeriod(animalGroup, managementPeriod,
                    animalComponent, farm);
            else
                result = _dairyCattleResultsService.GetResultsForManagementPeriod(animalGroup, managementPeriod,
                    animalComponent, farm);
            ...
            GrazingViewItem grazingViewItem)
        {
            ...
            var animalComponentEmissionsResults =
                results.SingleOrDefault(x => x.Component.Guid == grazingViewItem.AnimalComponentGuid);
            if (animalComponentEmissionsResults != null)
            {
                //Get all animal groups that have been placed on this field for grazing.
                var groupEmissionResults =
                    animalComponentEmissionsResults.EmissionResultsForAllAnimalGroupsInComponent.SingleOrDefault(x =>
                        x.AnimalGroup.Guid == grazingViewItem.AnimalGroupGuid);
            ...
            FieldSystemComponent fieldSystemComponent)
        {
            ...
            var grazingPeriods = managementPeriods
                .Where(x => fieldSystemComponent.IsGrazingManagementPeriodFromPasture(x)).ToList();
