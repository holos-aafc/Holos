...
{
    /// <summary>
    ///     Helper class that inherits and implements <see cref="IAnaerobicDigestionComponentHelper" />. The class handles
    ///     functions like AD component initialization, getting unique names for components etc.
    /// </summary>
    public class AnaerobicDigestionComponentHelper : IAnaerobicDigestionComponentHelper
    {
        ...
        /// <summary>
        ///     Initializes a new component. The method gives each component a unique name and sets the
        ///     <see cref="AnaerobicDigestionComponent.IsInitialized" /> flag.
        /// </summary>
        ...
        /// <summary>
        ///     Sets a unique name for each AD component. It takes in a collection of components as a parameter. The method then
        ///     sets a unique name based on whether
        ///     a name currently exists in the collection or not. The method starts with a base name and keeps on adding a number
        ///     to the end, comparing it to the collection, until a unique name is found. When a unique name is found,
        ...
        public string GetUniqueAnaerobicDigestionName(IEnumerable<AnaerobicDigestionComponent> components)
        {
            ...
            var uniqueName = Resources.EnumAnaerobicDigestion;
            while (components.Any(x => x.Name == uniqueName))
                uniqueName = $"{Resources.EnumAnaerobicDigestion} #{++componentNumber}";
            return uniqueName;
        }

        public AnaerobicDigestionComponent Replicate(AnaerobicDigestionComponent component,
            IEnumerable<AnimalComponentBase> replicatedAnimalComponents)
        {
        ...
        public void Replicate(ComponentBase source, ComponentBase destination,
            IEnumerable<AnimalComponentBase> replicatedAnimalComponents)
        {
            ...
            foreach (var animalComponent in replicatedAnimalComponents.Where(x =>
                         x.ComponentCategory != ComponentCategory.Sheep))
            foreach (var animalGroup in animalComponent.Groups)
            foreach (var managementPeriod in animalGroup.ManagementPeriods.Where(x =>
                         x.ManureDetails.StateType != ManureStateType.Pasture))
            foreach (var managementPeriodViewItem in from.ManagementPeriodViewItems)
                if (managementPeriodViewItem.AnimalGroup.Name == animalGroup.Name &&
                    managementPeriodViewItem.ManagementPeriod.Name == managementPeriod.Name)
                {
            ...
            to.ManagementPeriodViewItems =
                new ObservableCollection<ADManagementPeriodViewItem>(aDManagementPeriodViewItems);
            ...
            foreach (var cropResidueSubstrateViewItem in from.AnaerobicDigestionViewItem.CropResiduesSubstrateViewItems)
            {
                ...
                _cropResiduesSubstrateViewItemMapper.Map(cropResidueSubstrateViewItem,
                    copiedCropResidueSubstrateViewItem);
            ...
            foreach (var farmResidueSubstrateViewItem in from.AnaerobicDigestionViewItem.FarmResiduesSubstrateViewItems)
            {
                ...
                _farmResiduesSubstrateViewItemMapper.Map(farmResidueSubstrateViewItem,
                    copiedFarmResiduesSubstrateViewItem);
