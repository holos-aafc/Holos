...
namespace H.Core.Services.Initialization.Animals
{
    public partial class AnimalInitializationService
    {
        ...
        private List<DefaultManureCompositionData> SeedDefaultManureCompositionData(
            List<DefaultManureCompositionData> source)
        {
            var manureCompositionData = new List<DefaultManureCompositionData>();

            foreach (var item in source)
            {
                var copiedInstance = _defaultManureCompositionDataMapper
                    .Map<DefaultManureCompositionData, DefaultManureCompositionData>(item);
        ...
        /// <summary>
        ///     Initialize the default <see cref="DefaultManureCompositionData" /> for all <see cref="ManagementPeriod" />s
        ...
        public void InitializeManureCompositionData(Farm farm)
        {
            if (farm != null)
            {
                ...
                farm.DefaultManureCompositionData =
                    new ObservableCollection<DefaultManureCompositionData>(
                        SeedDefaultManureCompositionData(manureCompositionData));

                foreach (var managementPeriod in farm.GetAllManagementPeriods())
                {
                    var defaults = farm.GetManureCompositionData(managementPeriod.ManureDetails.StateType,
                        managementPeriod.AnimalType);

                    InitializeManureCompositionData(managementPeriod, defaults);
                }
            }
        }
        ...
        public void ReinitializeManureCompositionData(Farm farm)
        {
            if (farm != null)
                foreach (var managementPeriod in farm.GetAllManagementPeriods())
                {
                    var manureCompositionData = farm.GetManureCompositionData(managementPeriod.ManureDetails.StateType,
                        managementPeriod.AnimalType);

                    InitializeManureCompositionData(managementPeriod, manureCompositionData);
                }
        }
