...
namespace H.Core.Services.Initialization.Animals
{
    public partial class AnimalInitializationService
    {
        ...
        private List<Table_30_Default_Bedding_Material_Composition_Data> SeedDefaultBeddingCompositionData(
            List<Table_30_Default_Bedding_Material_Composition_Data> source)
        {
            var beddingMaterialCompositionData = new List<Table_30_Default_Bedding_Material_Composition_Data>();

            foreach (var item in source)
            {
                var copiedInstance = _defaultBeddingCompositionDataMapper
                    .Map<Table_30_Default_Bedding_Material_Composition_Data,
                        Table_30_Default_Bedding_Material_Composition_Data>(item);
        ...
        /// <summary>
        ///     Initialize the <see cref="Table_30_Default_Bedding_Material_Composition_Data" /> for all
        ...
        public void InitializeBeddingMaterial(Farm farm)
        {
            if (farm != null)
            {
                ...
                farm.DefaultsCompositionOfBeddingMaterials =
                    new ObservableCollection<Table_30_Default_Bedding_Material_Composition_Data>(
                        SeedDefaultBeddingCompositionData(data));
                ...
                foreach (var managementPeriod in farm.GetAllManagementPeriods())
                {
                    ...
                    InitializeBeddingMaterial(managementPeriod, beddingMaterialComposition);
                }
            }
        }
        ...
        public void ReinitializeBeddingMaterial(Farm farm)
        {
            if (farm != null)
                foreach (var managementPeriod in farm.GetAllManagementPeriods())
                    InitializeBeddingMaterial(managementPeriod, farm);
        }
            ...
            Table_30_Default_Bedding_Material_Composition_Data data)
        {
            if (managementPeriod != null && managementPeriod.HousingDetails != null && data != null)
            {
                managementPeriod.HousingDetails.TotalCarbonKilogramsDryMatterForBedding =
                    data.TotalCarbonKilogramsDryMatter;
                managementPeriod.HousingDetails.TotalNitrogenKilogramsDryMatterForBedding =
                    data.TotalNitrogenKilogramsDryMatter;
                managementPeriod.HousingDetails.TotalPhosphorusKilogramsDryMatterForBedding =
                    data.TotalPhosphorusKilogramsDryMatter;
        ...
        public void InitializeBeddingMaterialRate(Farm farm)
        {
            if (farm != null)
                foreach (var managementPeriod in farm.GetAllManagementPeriods())
                    InitializeBeddingMaterialRate(managementPeriod);
        }
