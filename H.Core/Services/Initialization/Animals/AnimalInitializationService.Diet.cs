    ...
    public partial class AnimalInitializationService
    {
        ...
        public void InitializeStartAndEndWeightsForCattle(Farm farm)
        {
            if (farm != null)
            {
                ...
                {
                    this.InitializeStartAndEndWeightsForCattle(managementPeriod);
                }
            }
        }
        ...
        {
            if (!(managementPeriod.AnimalType.GetCategory() == AnimalType.Beef || managementPeriod.AnimalType.GetCategory() == AnimalType.Dairy))
            {
                return;
            }
            var coefficients = _livestockCoefficientsBeefAndDairyCattleProvider.GetCoefficientsByAnimalType(managementPeriod.AnimalType);
