...
namespace H.Core.Services.Initialization.Animals
{
    public partial class AnimalInitializationService
    {
        ...
        public void InitializeDefaultEmissionFactors(Farm farm)
        {
            if (farm != null)
                foreach (var managementPeriod in farm.GetAllManagementPeriods())
                    InitializeDefaultEmissionFactors(farm, managementPeriod);
        }
        ...
        public void InitializeFeedingActivityCoefficient(Farm farm)
        {
            if (farm != null)
                foreach (var managementPeriod in farm.GetAllManagementPeriods())
                    InitializeFeedingActivityCoefficient(managementPeriod);
        }
                ...
                if (managementPeriod.AnimalType.IsBeefCattleType() || managementPeriod.AnimalType.IsDairyCattleType())
                {
                    managementPeriod.HousingDetails.ActivityCeofficientOfFeedingSituation =
                        _beefDairyCattleFeedingActivityCoefficientProvider
                            .GetByHousing(managementPeriod.HousingDetails.HousingType).FeedingActivityCoefficient;
                }
                else if (managementPeriod.AnimalType.IsSheepType())
                {
                    managementPeriod.HousingDetails.ActivityCeofficientOfFeedingSituation =
                        _feedingActivityCoefficientSheepProvider
                            .GetByHousing(managementPeriod.HousingDetails.HousingType).FeedingActivityCoefficient;
                }
                else
                {
                    Trace.TraceError($"{nameof(InitializeFeedingActivityCoefficient)}" +
                                     $" unable to get data for housing type: {managementPeriod.HousingDetails.HousingType}." +
        ...
        public void InitializeBaselineCoefficient(Farm farm)
        {
            if (farm != null)
                foreach (var managementPeriod in farm.GetAllManagementPeriods())
                    InitializeBaselineCoefficient(managementPeriod);
        }
                ...
                if (managementPeriod.AnimalType.IsBeefCattleType() || managementPeriod.AnimalType.IsDairyCattleType())
                    managementPeriod.HousingDetails.BaselineMaintenanceCoefficient =
                        _livestockCoefficientsBeefAndDairyCattleProvider
                            .GetCoefficientsByAnimalType(managementPeriod.AnimalType).BaselineMaintenanceCoefficient;
                else if (managementPeriod.AnimalType.IsSheepType())
                    managementPeriod.HousingDetails.BaselineMaintenanceCoefficient = _sheepProvider
                        .GetCoefficientsByAnimalType(managementPeriod.AnimalType).BaselineMaintenanceCoefficient;
            }
        }
        ...
        public void InitializeGainCoefficient(Farm farm)
        {
            if (farm != null)
                foreach (var managementPeriod in farm.GetAllManagementPeriods())
                    InitializeGainCoefficient(managementPeriod);
        }
        ...
        public void InitializeGainCoefficient(ManagementPeriod managementPeriod)
        {
            ...
            {
                if (managementPeriod.AnimalType.IsBeefCattleType() || managementPeriod.AnimalType.IsDairyCattleType())
                    managementPeriod.GainCoefficient = _livestockCoefficientsBeefAndDairyCattleProvider
                        .GetCoefficientsByAnimalType(managementPeriod.AnimalType).GainCoefficient;
                else if (managementPeriod.AnimalType.IsSheepType())
                    managementPeriod.GainCoefficient = _sheepProvider
                        .GetCoefficientsByAnimalType(managementPeriod.AnimalType).GainCoefficient;
            }
        }
