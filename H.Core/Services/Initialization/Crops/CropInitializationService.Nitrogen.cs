...
namespace H.Core.Services.Initialization.Crops
{
    public partial class CropInitializationService
    {
        ...
        public void InitializeNitrogenFixation(Farm farm)
        {
            ...
            foreach (var viewItem in viewItems) InitializeNitrogenFixation(viewItem);
        }
        ...
        {
            foreach (var viewItem in farm.GetAllCropViewItems()) InitializeNitrogenContent(viewItem, farm);
        }

        public void InitializeNitrogenContent(List<CropViewItem> viewItem, Farm farm)
        {
            foreach (var cropViewItem in viewItem) InitializeNitrogenContent(cropViewItem, farm);
        }
        ...
        public void InitializeNitrogenContentInStraw(CropViewItem viewItem, Farm farm)
        {
            ...
            if (residueData != null)
            {
                ...
                if (viewItem.CropType.IsPerennial()) viewItem.NitrogenContentInStraw = 0;
            }
        }
        ...
        public void InitializeNitrogenContent(CropViewItem viewItem, Farm farm)
        {
            ...
            if (residueData != null)
            {
                InitializeNitrogenContentInProduct(viewItem, farm);
                InitializeNitrogenContentInStraw(viewItem, farm);
                InitializeNitrogenContentInRoots(viewItem, farm);
                InitializeNitrogenContentInExtraroots(viewItem, farm);
            }

            // Assign N content values used for IPCC Tier 2
            InitializeIPCCNitrogenContent(viewItem, farm);
        }
