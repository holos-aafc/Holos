...
namespace H.Core.Services.Initialization.Crops
{
    public partial class CropInitializationService
    {
        ...
        /// </summary>
        /// <param name="farm">
        ///     The <see cref="Farm" /> containing the <see cref="CropViewItem" />s that will have reset their
        ///     carbon concentrations reset
        /// </param>
        public void InitializeCarbonConcentration(Farm farm)
        {
            var defaults = new Defaults();
            var viewItems = farm.GetAllCropViewItems();
            foreach (var viewItem in viewItems) InitializeCarbonConcentration(viewItem, defaults);
        }

        /// <summary>
        ///     Initialize the carbon concentration of the <see cref="CropViewItem" /> with the values in the
        ///     <see cref="Defaults" /> parameter
        /// </summary>
        ...
        {
            foreach (var viewItem in farm.GetAllCropViewItems()) InitializeBiomassCoefficients(viewItem, farm);
        }
        ...
        public void InitializeBiomassCoefficientProduct(CropViewItem viewItem, Farm farm)
        {
            ...
            if (residueData != null)
            {
                ...
                SetBasedOnHarvestMethod(viewItem, residueData);
            }
        }
        ...
        private void SetBasedOnHarvestMethod(CropViewItem viewItem,
            Table_7_Relative_Biomass_Information_Data residueData)
        {
            if (viewItem.HarvestMethod == HarvestMethods.Swathing ||
                viewItem.HarvestMethod == HarvestMethods.GreenManure || viewItem.HarvestMethod == HarvestMethods.Silage)
            {
                viewItem.BiomassCoefficientProduct =
                    residueData.RelativeBiomassProduct + residueData.RelativeBiomassStraw;
        ...
        public void InitializeBiomassCoefficients(CropViewItem viewItem, Farm farm)
        {
            ...
            if (residueData != null)
            {
                InitializeBiomassCoefficientProduct(viewItem, farm);
                InitializeBiomassCoefficientStraw(viewItem, farm);
                InitializeBiomassCoefficientRoots(viewItem, farm);
                InitializeBiomassCoefficientExtraroots(viewItem, farm);
        ...
        public void InitializeLumCMaxValues(CropViewItem cropViewItem, Farm farm)
        {
            if (!cropViewItem.CropType.IsPerennial() && !cropViewItem.CropType.IsGrassland() &&
                !cropViewItem.CropType.IsFallow() && !cropViewItem.IsBrokenGrassland) return;
            ...
            {
                var changeType =
                    _landManagementChangeHelper.GetPerennialCroppingChangeType(cropViewItem.PastPerennialArea,
                        cropViewItem.Area);
                ...
                lumCMax = _lumCMaxKValuesPerennialCroppingChangeProvider.GetLumCMax(ecozone,
                    farm.GeographicData.DefaultSoilData.SoilTexture, changeType);
                kValue = _lumCMaxKValuesPerennialCroppingChangeProvider.GetKValue(ecozone,
                    farm.GeographicData.DefaultSoilData.SoilTexture, changeType);
            }
            else if (cropViewItem.CropType.IsFallow())
            {
                var changeType =
                    _landManagementChangeHelper.GetFallowPracticeChangeType(cropViewItem.PastFallowArea,
                        cropViewItem.Area);

                lumCMax = _lumCMaxKValuesFallowPracticeChangeProvider.GetLumCMax(ecozone,
                    farm.GeographicData.DefaultSoilData.SoilTexture, changeType);
                kValue = _lumCMaxKValuesFallowPracticeChangeProvider.GetKValue(ecozone,
                    farm.GeographicData.DefaultSoilData.SoilTexture, changeType);
            }
