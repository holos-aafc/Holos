...
namespace H.Core.Calculators.Infrastructure
{
    public partial class ADCalculator : IADCalculator
    {
        ...
        public ADCalculator()
        {
            var substrateFlowMapperConfiguration = new MapperConfiguration(configuration =>
            {
            ...
            ADManagementPeriodViewItem adManagementPeriod, Farm farm)
        {
            ...
            substrateFlowRate.TotalSolidsFlowOfSubstrate =
                substrateFlowRate.TotalMassFlowOfSubstrate * (1 - moistureContent / 100.0);
            ...
            if (animalType.IsBeefCattleType() || animalType.IsDairyCattleType() || animalType.IsSheepType())
                ...
                // Equation 4.8.1-7
                substrateFlowRate.OrganicNitrogenFlowOfSubstrate = (dailyEmissions.AmountOfNitrogenExcreted -
            ...
            GroupEmissionsByDay previousDaysEmissions, Farm farm)
        {
            ...
            var manureComposition =
                farm.GetManureCompositionData(managementPeriod.ManureDetails.StateType, managementPeriod.AnimalType);
            var moistureContent = manureComposition.MoistureContent;
            if (animalType.IsBeefCattleType() || animalType.IsDairyCattleType() || animalType.IsPoultryType())
                // Equation 4.8.1-16
                totalMassFlowOfSubstrate =
                    (dailyEmissions.AdjustedAmountOfTanInStoredManureOnDay +
                     dailyEmissions.OrganicNitrogenCreatedOnDay) * 100 / nitrogenContentOfManure * fractionUsed;
            else
                // Equation 4.8.1-17
                // Sheep, swine, and other livestock
                totalMassFlowOfSubstrate = dailyEmissions.AccumulatedNitrogenAvailableForLandApplicationOnDay * 100 /
                    nitrogenContentOfManure * fractionUsed;
            ...
            if (managementPeriod.ManureDetails.StateType.IsLiquidManure())
            {
                ...
                var volatileSolidsOnPreviousDay =
                ...
                var b = dailyEmissions.AccumulatedVolume -
            ...
            {
                // Equation 4.8.1-21
                substrateFlowRate.NitrogenFlowOfSubstrate = (dailyEmissions.AdjustedAmountOfTanInStoredManureOnDay +
                                                             dailyEmissions.OrganicNitrogenCreatedOnDay) * fractionUsed;
                ...
                substrateFlowRate.ExcretedTanInSubstrate =
                    dailyEmissions.AdjustedAmountOfTanInStoredManureOnDay * fractionUsed;
            }
            else if (animalType.IsPoultryType())
            {
                ...
                // Equation 4.8.1-21
                substrateFlowRate.NitrogenFlowOfSubstrate =
                    dailyEmissions.NonAccumulatedNitrogenEnteringPoolAvailableInStorage * fractionUsed;

                // Equation 4.8.1-23
                substrateFlowRate.OrganicNitrogenFlowOfSubstrate = substrateFlowRate.NitrogenFlowOfSubstrate -
                                                                   dailyEmissions
                                                                       .AdjustedAmountOfTanInStoredManureOnDay;
        ...
        {
            var biodegradableFraction = GetBiodegradableFraction(substrateFlowRate);
            ...
            substrateFlowRate.MethaneProduction = substrateFlowRate.DegradedVolatileSolids *
                                                  (substrateFlowRate.BiomethanePotential / 1000.0);
            ...
            AnaerobicDigestionComponent component)
        {
            ...
            var nitrogenCoefficients =
                _solidLiquidSeparationCoefficientsProvider.GetSolidLiquidSeparationCoefficientInstance(
                    DigestateParameters.TotalNitrogen);
            ...
            digestorDailyOutput.CarbonLiquidFraction =
                (1 - carbonCoefficient) * digestorDailyOutput.FlowOfAllCarbonInDigestate;

            // Equation 4.8.4-14
            digestorDailyOutput.CarbonSolidFraction =
                carbonCoefficient * digestorDailyOutput.FlowOfAllCarbonInDigestate;
        }
            ...
            List<SubstrateFlowInformation> flowInformationForAllSubstrates)
        {
            ...
            foreach (var flowInformationForAllSubstrate in flowInformationForAllSubstrates)
            {
                ...
                var totalSolidsFlowOfSubstrate =
                    flowInformationForAllSubstrate.TotalSolidsFlowOfSubstrate - degradedVolatileSolids;
                ...
                var volatileSolidsFlowOfSubstrate = flowInformationForAllSubstrate.VolatileSolidsFlowOfSubstrate -
                                                    degradedVolatileSolids;
                if (volatileSolidsFlowOfSubstrate < 0) volatileSolidsFlowOfSubstrate = 0;
            ...
            digestorDailyOutput.FlowOfAllCarbonInDigestate =
                flowInformationForAllSubstrates.Sum(x => x.CarbonFlowInDigestate);
        }
            ...
            List<AnimalComponentEmissionsResults> animalComponentEmissionsResults)
        {
            ...
            var onFarmManureFlowRates = GetDailyManureFlowRates(farm, animalComponentEmissionsResults, component);
            var cropResidueFlowRates = GetDailyCropResidueFlowRates(component);
            var farmResidueFlowsRates = GetDailyFarmResidueFlowRates(component);
            var importedManureFlowRates = GetDailyImportedManureResidueFlowRates(component);
            ...
            {
                CalculateDailyBiogasProductionFromSingleSubstrate(substrateFlowInformation);
                CalculateFlowsInDigestateFromSingleSubstrate(substrateFlowInformation);
            }

            // Sum flows that occur on same day.
            var combinedDailyResults = CombineSubstrateFlowsOfSameTypeOnSameDay(dailyFlowRates, component, farm);
            ...
            foreach (var substrateViewItemBase in substrateViewItems)
            {
                ...
                for (var currentDate = startDate; currentDate < endDate; currentDate += TimeSpan.FromDays(1))
                {
                    ...
                    if (isCropResidue)
                    {
                        ...
                        substrateFlow.TotalSolidsFlowOfSubstrate =
                            substrateFlow.TotalMassFlowOfSubstrate * (cropTotalSolids / 1000.0);
                    }
                    else
                    {
                        substrateFlow.TotalSolidsFlowOfSubstrate = substrateFlow.TotalMassFlowOfSubstrate *
                                                                   (substrateViewItemBase.TotalSolids / 1000.0);
                    }
                    ...
                    {
                        substrateFlow.VolatileSolidsFlowOfSubstrate = substrateFlow.TotalSolidsFlowOfSubstrate *
                                                                      (substrateViewItemBase.VolatileSolids / 100.0);
                    }
                    else
                    {
                        // Manure
                        ...
                        if (type == SubstrateType.ImportedManure)
                            // Equation 4.8.1-29
                            substrateFlow.VolatileSolidsFlowOfSubstrate = substrateViewItemBase.FlowRate *
                                                                          substrateViewItemBase.VolatileSolidsContent;
                    }
                    ...
                    if (isNonManureResidue)
                    {
                        substrateFlow.NitrogenFlowOfSubstrate = substrateViewItemBase.FlowRate *
                                                                (substrateViewItemBase.TotalNitrogen / 1000.0);
                    }
                    else
                    {
                        // Manure
                        ...
                        if (type == SubstrateType.ImportedManure)
                            // Equation 4.8.1-30
                            substrateFlow.NitrogenFlowOfSubstrate = substrateViewItemBase.FlowRate *
                                                                    (substrateViewItemBase.NitrogenContent / 100.0);
                    }
                    ...
                    else
                    {
                        ...
                        if (type == SubstrateType.ImportedManure)
                            // Equation 4.8.1-31
                            substrateFlow.CarbonFlowOfSubstrate = substrateViewItemBase.FlowRate *
                                                                  (substrateViewItemBase.CarbonContent / 100.0);
                    }
        ...
        {
            return GetDailyFlowRatesForSubstrateTypes(
        ...
        public List<SubstrateFlowInformation> GetDailyImportedManureResidueFlowRates(
            AnaerobicDigestionComponent component)
        {
            return GetDailyFlowRatesForSubstrateTypes(
                         ...
                         .EmissionResultsForAllAnimalGroupsInComponent)
            {
                ...
                foreach (var groupEmissionsByMonth in animalGroupResults.GroupEmissionsByMonths)
                        ...
                        .Contains(groupEmissionsByMonth.MonthsAndDaysData.ManagementPeriod))
                    {
                        ...
                        for (var i = 0; i < groupEmissionsByMonth.DailyEmissions.Count; i++)
                        {
                            var currentDayEmissions = groupEmissionsByMonth.DailyEmissions.ElementAt(i);

                            var flowRates = GetStoredManureFlowRate(
                                component,
                                currentDayEmissions,
        ...
        /// <summary>
        ///     We take all flows and combine any flows that occur on the same day and that are of the same type of substrate. For
        ///     example,
        ///     if there are two flows of dairy manure, we combine them together into a single flow. If the substrate are not of
        ///     the same type
            ...
            List<SubstrateFlowInformation> substrateFlows, AnaerobicDigestionComponent component, Farm farm)
        {
            ...
            foreach (var byDayGroup in flowsGroupedByDay)
            {
                ...
                CalculateTotalProductionFromAllSubstratesOnSameDay(dailyOutput, flowsForDate);
                CalculateTotalBiogasProductionFromAllSubstratesOnSameDay(dailyOutput, flowsForDate);
                CalculateLiquidSolidSeparation(dailyOutput, component);
                CalculateAmountsForLandApplication(dailyOutput);
                CalculateDigestateStorageEmissions(farm, dailyOutput.Date, component, dailyOutput);

                // Equation 4.8.6-1
                dailyOutput.TotalNitrogenInDigestateAvailableForLandApplication =
                    dailyOutput.FlowRateOfTotalNitrogenInDigestate -
                    (CoreConstants.ConvertToN(dailyOutput.N2OEmissionsDuringStorage) +
                     CoreConstants.ConvertToNH3N(dailyOutput.AmmoniaEmissionsDuringStorage));
                ...
                var totalCarbon = flowOfCarbonInDigestate -
                                  CoreConstants.ConvertToC(dailyOutput.MethaneEmissionsDuringStorage);
        ...
        protected double GetBiodegradableFraction(SubstrateFlowInformation substrateFlowInformation)
        {
            if (substrateFlowInformation.SubstrateType == SubstrateType.FarmResidues ||
                substrateFlowInformation.SubstrateType == SubstrateType.CropResidues) return 0.23;

            if (substrateFlowInformation.AnimalType.IsDairyCattleType()) return 0.4;

            if (substrateFlowInformation.AnimalType.IsSwineType()) return 0.7;
