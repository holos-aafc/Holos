...
namespace H.Core.Calculators.Carbon
{
    public class ICBMCarbonInputCalculator : CarbonInputCalculatorBase, IICBMCarbonInputCalculator
    {
            ...
            Farm farm, List<AnimalComponentEmissionsResults> animalResults)
        {
            ...
            currentYearViewItem.PlantCarbonInAgriculturalProduct = CalculatePlantCarbonInAgriculturalProduct(
                previousYearViewItem,
                currentYearViewItem,
                farm);

            currentYearViewItem.CarbonInputFromProduct = CalculateCarbonInputFromProduct(
                previousYearViewItem,
                currentYearViewItem,
                nextYearViewItem,
                farm);
            ...
            if (isNonSwathingGrazingScenario)
            {
                ...
                currentYearViewItem.CarbonInputFromProduct =
                    (currentYearViewItem.TotalCarbonLossesByGrazingAnimals -
                     currentYearViewItem.TotalCarbonUptakeByAnimals) / currentYearViewItem.Area;
                ...
                var moistureContent = currentYearViewItem.GrazingViewItems.Any()
                    ...
                    : 1;
                var totalYieldForArea = currentYearViewItem.TotalCarbonLossesByGrazingAnimals /
                                        farm.Defaults.CarbonConcentration / (1 - moistureContent / 100.0);

                // Convert to per hectare
                currentYearViewItem.Yield = totalYieldForArea / currentYearViewItem.Area;
            }

            currentYearViewItem.CarbonInputFromStraw = CalculateCarbonInputFromStraw(
                previousYearViewItem,
                currentYearViewItem,
                farm);

            currentYearViewItem.CarbonInputFromRoots = CalculateCarbonInputFromRoots(
                previousYearViewItem,
                currentYearViewItem,
                farm);

            currentYearViewItem.CarbonInputFromExtraroots = CalculateCarbonInputFromExtraroot(
                previousYearViewItem,
                currentYearViewItem,
                farm);

            currentYearViewItem.AboveGroundCarbonInput = CalculateTotalAboveGroundCarbonInput(
                currentYearViewItem,
                farm);

            var supplementalFeedingAmount = CalculateInputsFromSupplementalHayFedToGrazingAnimals(
                previousYearViewItem,
                currentYearViewItem,
                nextYearViewItem,
                farm);
            ...
            currentYearViewItem.BelowGroundCarbonInput = CalculateTotalBelowGroundCarbonInput(
                currentYearViewItem,
                farm);

            currentYearViewItem.ManureCarbonInputsPerHectare =
                manureService.GetTotalManureCarbonInputsForField(farm, currentYearViewItem.Year, currentYearViewItem);

            if (farm.CropHasGrazingAnimals(currentYearViewItem))
                currentYearViewItem.ManureCarbonInputsPerHectare +=
                    currentYearViewItem.TotalCarbonInputFromManureFromAnimalsGrazingOnPasture;

            currentYearViewItem.DigestateCarbonInputsPerHectare =
                digestateService.GetTotalDigestateCarbonInputsForField(farm, currentYearViewItem.Year,
                    currentYearViewItem);
            currentYearViewItem.DigestateCarbonInputsPerHectareFromApplicationsOnly =
                currentYearViewItem.GetTotalCarbonFromAppliedDigestate(ManureLocationSourceType.Livestock) /
                currentYearViewItem.Area;

            currentYearViewItem.TotalCarbonInputs = currentYearViewItem.AboveGroundCarbonInput +
            ...
            Farm farm)
        {
            ...
            if (currentYearViewItem.CropType.IsFallow() ||
                currentYearViewItem.CropType == CropType.NotSelected) return 0;

            // Old farms fix
            if (currentYearViewItem.MoistureContentOfCrop > 1) currentYearViewItem.MoistureContentOfCrop /= 100;
            ...
            var isAllProductReturned = Math.Abs(currentYearViewItem.PercentageOfProductYieldReturnedToSoil - 100) <
                                       double.Epsilon;
            ...
            var isCustomYieldAndNoHarvestAndNoGrazing =
                isCustomYieldAssignmentMethod && hasHarvest == false && isGrazed == false;
            ...
            if (isAllProductReturned || isSwathing || isGreenManure || isCustomYieldAndIsGrazed ||
                isCustomYieldAndNoHarvestAndNoGrazing)
            {
            ...
            Farm farm)
        {
            ...
            var proportionOfPrecipitationMayThroughSeptember =
                farm.ClimateData.ProportionOfPrecipitationFallingInMayThroughSeptember(year);

            var result = CalculateProductivity(
                totalPrecipitationForTheYear,
                totalEvapotranspirationForTheYear,
                proportionOfPrecipitationMayThroughSeptember,
                carbonConcentration);
        ...
        {
            var production = 2.973 + 0.00453 * annualPrecipitation + -0.00259 * annualPotentialEvapotranspiration +
            ...
            Farm farm)
        {
            ...
            var estimatedPlantCarbonInAgriculturalProductInNextYear =
                EstimatePlantCarbonInAgriculturalProductForNextYear(
                    currentYearViewItem,
                    farm);
            ...
            if (currentYearViewItem.YearInPerennialStand == 1)
            {
                ...
                if (currentYearViewItem.PlantCarbonInAgriculturalProduct > 0)
                {
                    ...
                    carbonInputFromProduct = currentYearViewItem.PlantCarbonInAgriculturalProduct *
                                             (currentYearViewItem.PercentageOfProductYieldReturnedToSoil / 100);
                ...
                else if (currentYearViewItem.PlantCarbonInAgriculturalProduct == 0 && nextYearViewItem != null &&
                         (nextYearViewItem.PlantCarbonInAgriculturalProduct > 0 || nextYearViewItem.Yield > 0))
                {
                    /* Equation 2.1.2-21
                     *
                    ...
                    if (nextYearViewItem.PlantCarbonInAgriculturalProduct > 0)
                        ...
                        plantCarbonInAgriculturalProductForNextYear = CalculatePlantCarbonInAgriculturalProduct(
                            null,
                            nextYearViewItem,
                            farm);
                    ...
                    // Equation 2.1.2-23
                    var thisYearsPlantCarbonInAgriculturalProductAsAFractionOfNextYears =
                        plantCarbonInAgriculturalProductForNextYear *
                        farm.Defaults.EstablishmentGrowthFactorFractionForPerennials;

                    // Equation 2.1.2-24
                    carbonInputFromProduct = thisYearsPlantCarbonInAgriculturalProductAsAFractionOfNextYears *
                                             (currentYearViewItem.PercentageOfProductYieldReturnedToSoil / 100);
                    ...
                    currentYearViewItem.PlantCarbonInAgriculturalProduct =
                        thisYearsPlantCarbonInAgriculturalProductAsAFractionOfNextYears;
                }
                else
                {
                    /*
            ...
            else
            {
                ...
                else
                {
                    ...
                    var estimatedPlantC = estimatedPlantCarbonInAgriculturalProductInNextYear *
                                          farm.Defaults.EstablishmentGrowthFactorFractionForPerennials;

                    carbonInputFromProduct =
                        estimatedPlantC * (currentYearViewItem.PercentageOfProductYieldReturnedToSoil / 100);
            ...
            Farm farm)
        {
            if (currentYearViewItem.CropType.IsPerennial())
                return CalculateAboveGroundCarbonInputFromPerennials(
                    previousYearViewItem,
                    currentYearViewItem,
                    nextYearViewItem,
                    farm);

            var result = currentYearViewItem.PlantCarbonInAgriculturalProduct *
                         (currentYearViewItem.PercentageOfProductYieldReturnedToSoil / 100.0);
            ...
            Farm farm)
        {
            if (currentYearViewItem.CropType.IsFallow() || // No inputs from fallow fields
                ...
                currentYearViewItem.HarvestMethod ==
                HarvestMethods
                    .GreenManure || // In these two harvest method cases, the residue fractions for product and straw are combined and so the inputs from straw are omitted
                currentYearViewItem.HarvestMethod == HarvestMethods.Swathing ||
                currentYearViewItem.CropType
                    .IsPerennial()) // All above ground inputs from perennials are from the product and not the straw
                return 0;
            ...
            if (Math.Abs(currentYearViewItem.BiomassCoefficientProduct) < double.Epsilon) return 0;
            ...
            Farm farm)
        {
            ...
            if (currentYearViewItem.BiomassCoefficientProduct == 0) return 0;

            // Equation 2.1.2-28
            var carbonInputFromRoots = currentYearViewItem.PlantCarbonInAgriculturalProduct *
                                       (currentYearViewItem.BiomassCoefficientRoots /
                                        currentYearViewItem.BiomassCoefficientProduct) *
                                       (currentYearViewItem.PercentageOfRootsReturnedToSoil / 100.0);
            if (carbonInputFromRoots < 450) carbonInputFromRoots = 450;

            if (currentYearViewItem.YearInPerennialStand == 1) return carbonInputFromRoots;
                ...
                previousYearViewItem.PerennialStandGroupId.Equals(currentYearViewItem.PerennialStandGroupId))
            {
                ...
                carbonInputFromRoots = previousYearViewItem.CarbonInputFromRoots +
                                       previousYearViewItem.CarbonInputFromRoots * (19.35 / 100.0);
            ...
            Farm farm)
        {
            ...
            if (currentYearViewItem.CropType.IsPerennial() || currentYearViewItem.CropType == CropType.GrassSilage)
                return CalculateCarbonInputFromRootsForPerennials(
                    previousYearViewItem,
                    currentYearViewItem,
                    farm);
            ...
            if (currentYearViewItem.HarvestMethod == HarvestMethods.GreenManure ||
                currentYearViewItem.HarvestMethod == HarvestMethods.Swathing)
                return CalculateCarbonInputFromRootsForGreenManureOrSwathing(
                    previousYearViewItem,
                    currentYearViewItem,
                    farm);
        ...
        public double CalculateCarbonInputFromRootsForGreenManureOrSwathing(
            ...
            Farm farm)
        {
            var combinedBiomassCoefficientOfProduct = currentYearViewItem.BiomassCoefficientProduct +
                                                      currentYearViewItem.BiomassCoefficientStraw;
            if (combinedBiomassCoefficientOfProduct == 0) return 0;

            var greenManureResult = currentYearViewItem.PlantCarbonInAgriculturalProduct *
                                    (currentYearViewItem.BiomassCoefficientRoots /
                                     combinedBiomassCoefficientOfProduct) *
                                    (currentYearViewItem.PercentageOfRootsReturnedToSoil / 100);
            ...
            Farm farm)
        {
            ...
            if (currentYearViewItem.CropType.IsPerennial() || currentYearViewItem.CropType == CropType.GrassSilage)
                return CalculateCarbonInputFromExtrarootsForPerennials(
                    previousYearViewItem,
                    currentYearViewItem,
                    farm);
            ...
            if (currentYearViewItem.HarvestMethod == HarvestMethods.GreenManure ||
                currentYearViewItem.HarvestMethod == HarvestMethods.Swathing)
                return CalculateCarbonInputFromExtrarootsForGreenManureOrSwathing(
                    previousYearViewItem,
                    currentYearViewItem,
                    farm);
            ...
            var result = currentYearViewItem.PlantCarbonInAgriculturalProduct *
                         (currentYearViewItem.BiomassCoefficientExtraroot /
                          currentYearViewItem.BiomassCoefficientProduct);
            ...
            Farm farm)
        {
            ...
            // Equation 2.1.2-29
            // Equation 2.1.2-31
            var carbonInputFromExtraroots = currentYearViewItem.PlantCarbonInAgriculturalProduct *
                                            (currentYearViewItem.BiomassCoefficientExtraroot /
                                             currentYearViewItem.BiomassCoefficientProduct);
            ...
            Farm farm)
        {
            ...
            if (cropViewItem.HarvestMethod == HarvestMethods.GreenManure ||
                cropViewItem.HarvestMethod == HarvestMethods.Swathing) return cropViewItem.CarbonInputFromProduct;
