...
namespace H.Core.Calculators.Carbon
{
    public class IPCCTier2CarbonInputCalculator : CarbonInputCalculatorBase, IIPCCTier2CarbonInputCalculator
    {
        ...
        /// <summary>
        ///     The IPCC Tier 2 approach can only estimate carbon inputs for a small set of crop types. If a crop type does not
        ///     have values for intercept, slope, etc.
        ...
        public void AssignInputs(CropViewItem viewItem, Farm farm,
            List<AnimalComponentEmissionsResults> animalComponentEmissionsResults)
        {
            ...
            var harvestIndex = CalculateHarvestIndex(
                slope,
                viewItem.Yield,
                intercept,
            ...
            if (viewItem.HarvestMethod == HarvestMethods.Swathing && farm.CropHasGrazingAnimals(viewItem))
                viewItem.PercentageOfProductYieldReturnedToSoil =
                    100 - viewItem.GetAverageUtilizationFromGrazingAnimals();

            viewItem.AboveGroundResidueDryMatter = CalculateAboveGroundResidueDryMatter(harvestIndex, viewItem);

            viewItem.AboveGroundResidueDryMatterExported =
                CalculateAboveGroundResidueDryMatterExported(harvestIndex, viewItem);

            var fractionRenewed = viewItem.CropType.IsAnnual() ? 1 : 1 / viewItem.PerennialStandLength;

            var finalAboveGroundResidue = CalculateAnnualAboveGroundResidue(
                viewItem.AboveGroundResidueDryMatter,
                viewItem.Area,
                0,
                0,
                0);
            ...
            viewItem.BelowGroundResidueDryMatter = CalculateBelowGroundResidueDryMatter(cropData.RSTRatio,
                fractionRenewed,
                harvestIndex,
                viewItem);
            ...
            viewItem.BelowGroundCarbonInput =
                viewItem.BelowGroundResidueDryMatter * BelowGroundCarbonContent / viewItem.Area;

            if (farm.IsCommandLineMode == false)
            {
                viewItem.ManureCarbonInputsPerHectare =
                    manureService.GetTotalManureCarbonInputsForField(farm, viewItem.Year, viewItem);
            ...
            viewItem.DigestateCarbonInputsPerHectare =
                digestateService.GetTotalDigestateCarbonInputsForField(farm, viewItem.Year, viewItem);
            ...
            viewItem.TotalCarbonInputs = viewItem.AboveGroundCarbonInput + viewItem.BelowGroundCarbonInput +
                                         viewItem.ManureCarbonInputsPerHectare +
                                         viewItem.DigestateCarbonInputsPerHectare;
        }
            ...
            CropViewItem viewItem)
        {
            ...
            if (viewItem.HasGrazingViewItems)
                moistureContentOfCropAsPercentage =
            ...
            double combustionFactor)
        {
            ...
            return aboveGroundResidueDryMatterForCrop * area *
                   (1 - fractionRemoved - fractionBurned * combustionFactor);
        }
        ...
        ///     Equation 2.2.2-6
        /// </summary>
        /// <param name="shootToRootRatio">
        ///     Ratio of below-ground root biomass to above-ground shoot biomass (RS(T)) (kg dm ha^-1
        ///     (kg dm ha^-1)^-1)
        /// </param>
            ...
            CropViewItem cropViewItem)
        {
            ...
            if (harvestIndex <= 0) harvestIndex = 1;
