...
{
    /// <summary>
    ///     A service class to calculate inputs related to N. This will route calculations to the IPCC Tier 2 or ICBM
    ...
    public partial class NitrogenService : INitrogenService
    {
        ...
        public void AssignNitrogenInputs(List<CropViewItem> viewItems, Farm farm)
        {
            if (farm.IsCommandLineMode)
                ProcessCommandLineItems(viewItems, farm);
            else
                for (var i = 0; i < viewItems.Count; i++)
                {
                    ...
                    AssignNitrogenInputs(tuple.CurrentYearViewItem, farm, tuple.PreviousYearViewItem);
                }
        }

        public void AssignNitrogenInputs(AdjoiningYears adjoiningYears, Farm farm)
        {
            AssignNitrogenInputs(adjoiningYears.CurrentYearViewItem, farm, adjoiningYears.PreviousYearViewItem);
        }

        public void AssignNitrogenInputs(CropViewItem currentYearViewItem, Farm farm, CropViewItem previousYearViewItem)
        {
            currentYearViewItem.AbovegroundNitrogenInputs =
                CalculateAboveGroundResidueNitrogen(farm, currentYearViewItem, previousYearViewItem);
            currentYearViewItem.BelowgroundNitrogenInputs =
                CalculateBelowGroundResidueNitrogen(farm, currentYearViewItem, previousYearViewItem);
        }
        ...
        {
            if (CanCalculateNitrogenInputsUsingIpccTier2(currentYearViewItem) &&
            ...
            CropViewItem previousYearViewItem)
        {
            if (CanCalculateNitrogenInputsUsingIpccTier2(currentYearViewItem) &&
                farm.Defaults.CarbonModellingStrategy == CarbonModellingStrategies.IPCCTier2)
                return _ipccNitrogenInputCalculator.CalculateTotalBelowGroundResidueNitrogenUsingIpccTier2(
                    currentYearViewItem, previousYearViewItem);
        ...
        public double CalculateCropResidueExportNitrogen(CropViewItem cropViewItem)
        {
            if (cropViewItem.CropType.IsPerennial())
            {
                ...
                return _ipccNitrogenInputCalculator.CalculateTotalNitrogenFromExportedResidues(dryMatterPerHectare,
                    cropViewItem.NitrogenContentInProduct);
            }

            // Use IPCC Tier 2
            return _ipccNitrogenInputCalculator.CalculateTotalNitrogenFromExportedResidues(
                cropViewItem.AboveGroundResidueDryMatterExported, cropViewItem.NitrogenContentInStraw);
        }

        /// <summary>
        ///     Once N inputs have been determined for all crops, this method will check if there are one or more crops grown in
        ///     the same year. If there is, the total N
        ...
        public void CombineNitrogenInputs(Farm farm, List<CropViewItem> viewItems)
        {
            ...
            foreach (var year in distinctYears)
            {
                ...
                var totalAbovegroundNitrogenFromCoverCrops = SumAbovegroundNitrogen(secondaryCropsForYear);
                var totalBelowgroundNitrogenFromCoverCrops = SumBelowgroundNitrogen(secondaryCropsForYear);

                /*
                 * Sum up the main crop and cover crop N inputs
                 */

                mainCropForYear.CombinedAboveGroundResidueNitrogen = mainCropForYear.AbovegroundNitrogenInputs +
                                                                     totalAbovegroundNitrogenFromCoverCrops;
                mainCropForYear.CombinedBelowGroundResidueNitrogen = mainCropForYear.BelowgroundNitrogenInputs +
                                                                     totalBelowgroundNitrogenFromCoverCrops;
            }
        }
