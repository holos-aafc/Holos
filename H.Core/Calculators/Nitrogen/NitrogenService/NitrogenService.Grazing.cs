...
namespace H.Core.Calculators.Nitrogen.NitrogenService
{
    public partial class NitrogenService
    {
            ...
            List<AnimalComponentEmissionsResults> results)
        {
            ...
            var grazingViewItems = fieldSystemComponent.CropViewItems.Where(y => y.CropType == cropViewItem.CropType)
                .SelectMany(x => x.GrazingViewItems).ToList();
            ...
            foreach (var grazingViewItem in grazingItems)
            {
                var emissionsFromGrazingAnimals =
                    _animalService.GetGroupEmissionsFromGrazingAnimals(results, grazingViewItem);
                foreach (var groupEmissionsByMonth in emissionsFromGrazingAnimals)
                {
                    ...
                    totalN2ON += groupEmissionsByMonth.MonthlyManureDirectN2ONEmission +
                                 groupEmissionsByMonth.MonthlyManureIndirectN2ONEmission;
                }
            }

            var result =
                (totalNitrogenExcretedByAnimals -
                 (totalN2ON + CoreConstants.ConvertToNH3N(totalAmmoniaEmissions) + totalLeaching)) / cropViewItem.Area;
            ...
            IEnumerable<AnimalComponentEmissionsResults> results, List<CropViewItem> cropViewItems)
        {
            foreach (var cropViewItem in cropViewItems)
                cropViewItem.TotalNitrogenInputFromManureFromAnimalsGrazingOnPasture =
                    CalculateManureNitrogenInputsFromGrazingAnimals(fieldSystemComponent, cropViewItem,
                        results.ToList());
        }
    }
}