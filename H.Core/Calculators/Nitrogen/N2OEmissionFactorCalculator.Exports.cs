    ...
    public partial class N2OEmissionFactorCalculator
    {
        ...
        public double CalculateTotalDirectN2ONFromExportedManureByYear(Farm farm, int year)
        {
            ...
            foreach (var manureExportViewItem in farm.ManureExportViewItems.Where(x => x.DateOfExport.Year == year))
                result += CalculateTotalDirectN2ONFromExportedManure(farm, manureExportViewItem);
        ...
        ///     Equation 2.6.5-6
        ///     Equation 2.7.4-6
        ///     Note: These equations slightly differ from what is in the algorithm document. In the algorithm document there is a
        ///     placeholder for
        ...
        public double CalculateTotalDirectN2ONFromExportedManure(Farm farm, ManureExportViewItem manureExportViewItem)
        {
            ...
            var organicNitrogenEmissionFactor =
                CalculateOrganicNitrogenEmissionFactor(farm, manureExportViewItem.DateOfExport.Year);
            var emissionsForFarm =
                CalculateTotalDirectN2ONFromExportedManure(totalNitrogen, organicNitrogenEmissionFactor);
        ...
        ///     Equation 2.6.9-24
        ///     Equation 2.7.8-24
        ///     Note: Can't attribute indirect N2O emissions from exported manure to any one particular field. The following
        ///     amounts are not reported on the
        ///     multi year carbon modelling report. For this reason, only the amount from 2.7.8-24 will be shown and only on the
        ///     detailed emissions from (export emissions section)
        ///     (kg N2O-N year^-1)
        /// </summary>
        public double CalculateTotalIndirectN2ONFromExportedManure(Farm farm, int year)
        {
            ...
            var totalLeachingEmissionsFromExportedManure = CalculateTotalLeachingN2ONFromExportedManure(farm, year);
            var totalVolatilizationEmissionsFromExportedManure =
        ...
        public double CalculateTotalIndirectN2ONFromExportedManure(Farm farm, ManureExportViewItem manureExportViewItem)
        {
            ...
            var leachingEmissions = CalculateLeachingEmissionsFromManure(farm, totalNitrogenExported, year);
            var volatilizationEmissions = CalculateVolatilizationEmissionsFromExportedManure(farm,
                totalNitrogenExported, manureExportViewItem.AnimalType, manureExportViewItem.DateOfExport);
        ...
        public double CalculateTotalDirectN2ONFromExportedManureForFarmAndYear(Farm farm, int year)
        {
            ...
            result = CalculateTotalDirectN2ONFromExportedManureByYear(farm, year) *
        ...
        public double CalculateTotalLeachingN2ONFromExportedManure(Farm farm, int year)
        {
            ...
            var result = CalculateLeachingEmissionsFromManure(farm, nitrogenFromExportedManure, year);
        ...
        public double CalculateNH3NLossFromManureExports(Farm farm, int year, ManureExportViewItem manureExportViewItem)
        {
            ...
            var result = CalculateAmmoniaLossFromManureExports(farm, manureExportViewItem.AnimalType, year,
                nitrogenFromExportedManure);
        ...
        ///     (kg NH3-N)
        /// </summary>
        public double CalculateAdjustedNH3NLossFromManureExports(Farm farm, int year,
            ManureExportViewItem manureExportViewItem)
        {
            var result = 0d;

            var ammonia = CalculateNH3NLossFromManureExports(farm, year, manureExportViewItem);
            var totalNitrogenExported = _manureService.GetTotalNitrogenFromExportedManure(manureExportViewItem);
            var volatilization = CalculateVolatilizationEmissionsFromExportedManure(farm, totalNitrogenExported,
                manureExportViewItem.AnimalType, manureExportViewItem.DateOfExport);
        ...
        public double CalculateAdjustedNH3NLossFromManureExports(Farm farm, ManureExportViewItem manureExportViewItem)
        {
            ...
            var emissions = CalculateAdjustedNH3NLossFromManureExports(farm, year, manureExportViewItem);

            result = emissions / farm.GetTotalAreaOfFarm(false, year);
        ...
        ///     (kg NH3-N)
        /// </summary>
        private double CalculateAmmoniaLossFromManureExports(Farm farm, AnimalType animalType, int year,
            double totalNitrogen)
        {
            var factors = GetLandApplicationFactors(farm, animalType, year);
            ...
            int year)
        {
            ...
            var totalNitrogenFromExports = CalculateTotalNitrogenFromExportedManure(year, farm);
        ...
        private double CalculateVolatilizationEmissionsFromExportedManure(Farm farm,
            ...
            DateTime dateTime)
        {
            return CalculateVolatilizationEmissionsFromExportedManure(farm, totalNitrogen, animalType, dateTime.Year);
        }
            ...
            int year)
        {
            ...
            var ammonia = CalculateAmmoniaLossFromManureExports(farm, animalType, year, totalNitrogen);
        ...
        ///     (kg N2O-N ha^-1)
        /// </summary>
        public double CalculateVolatilizationEmissionsFromExportedManure(Farm farm,
            ManureExportViewItem manureExportViewItem)
        {
            var animalType = manureExportViewItem.AnimalType;
            var year = manureExportViewItem.DateOfExport.Year;
            var factors = GetLandApplicationFactors(farm, animalType, manureExportViewItem.DateOfExport.Year);
            var totalNitrogen = _manureService.GetTotalNitrogenFromExportedManure(manureExportViewItem);
            ...
            var result = emissions / farm.GetTotalAreaOfFarm(false, year);
        ...
        public double CalculateTotalNitrateLeachedFromExportedManureForFarmAndYear(Farm farm, int year)
        {
            ...
            var result = CalculateTotalNitrateLeachingEmissions(farm, exportedManureNitrogen, year);
        ...
        ///     (kg NO3-N ha^-1)
        /// </summary>
        public double CalculateTotalNitrateLeachedFromExportedManure(Farm farm,
            ManureExportViewItem manureExportViewItem)
        {
            var exportedManureNitrogen = _manureService.GetTotalNitrogenFromExportedManure(manureExportViewItem);
            var year = manureExportViewItem.DateOfExport.Year;

            var emissions = CalculateTotalNitrateLeachingEmissions(farm, exportedManureNitrogen, year);
            var result = emissions / farm.GetTotalAreaOfFarm(false, year);
