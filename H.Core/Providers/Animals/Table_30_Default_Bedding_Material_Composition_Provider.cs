...
namespace H.Core.Providers.Animals
{
    ...
    public class Table_30_Default_Bedding_Material_Composition_Provider
    {
        public Table_30_Default_Bedding_Material_Composition_Provider()
        {
            ...
            Data.Add(new Table_30_Default_Bedding_Material_Composition_Data
            {
        ...
        public List<Table_30_Default_Bedding_Material_Composition_Data> Data { get; } =
            new List<Table_30_Default_Bedding_Material_Composition_Data>();
        ...
        {
            if (beddingMaterialType == BeddingMaterialType.None) return 0;

            if (housingType.IsPasture()) return 0;

            if (animalType.IsYoungType()) return 0;

            if (animalType.IsBeefCattleType())
            {
                if (beddingMaterialType == BeddingMaterialType.Straw)
                {
                    if (housingType.IsFeedlot()) return 1.5;

                    if (housingType.IsBarn()) return 3.5;
                }

                if (beddingMaterialType == BeddingMaterialType.WoodChip)
                {
                    if (housingType.IsFeedlot()) return 3.6;

                    if (housingType.IsBarn()) return 5.0;
                }
            }

            if (animalType.IsDairyCattleType())
                if (housingType.IsTieStall() || housingType.IsFreeStall() ||
                    housingType == HousingType.DryLot) // Currently, all housing types have same rates for bedding types
                {
                    if (beddingMaterialType == BeddingMaterialType.Sand) return 24.3;

                    if (beddingMaterialType == BeddingMaterialType.SeparatedManureSolid) return 0;

                    if (beddingMaterialType == BeddingMaterialType.StrawLong) return 0.7;

                    if (beddingMaterialType == BeddingMaterialType.StrawChopped) return 0.7;

                    if (beddingMaterialType == BeddingMaterialType.Shavings) return 2.1;

                    if (beddingMaterialType == BeddingMaterialType.Sawdust) return 2.1;
                }

            // Footnote 8 for sheep value reference.
            if (animalType.IsSheepType()) return 0.57;

            if (animalType.IsSwineType())
            {
                if (beddingMaterialType == BeddingMaterialType.StrawLong) return 0.70;
            ...
            if (animalType.IsPoultryType())
            {
                if (beddingMaterialType == BeddingMaterialType.Sawdust ||
                    ...
                    beddingMaterialType == BeddingMaterialType.Shavings)
                {
                    if (animalType == AnimalType.Broilers) return 0.0014;

                    if (animalType == AnimalType.ChickenPullets) return 0.0014;

                    if (animalType == AnimalType.Layers || animalType == AnimalType.ChickenHens) return 0.0028;

                    if (animalType.IsTurkeyType()) return 0.011;
            ...
            Trace.TraceError(
                $"Unknown default bedding rate for {animalType.GetDescription()}, {housingType.GetDescription()}, and {beddingMaterialType.GetHashCode()}. Returning default value of 1.");
