...
namespace H.Core.Providers.Animals
{
    ...
    public class Table_29_Default_Manure_Excreted_Provider
    {
        ...
        private void ReadFile()
        {
            ...
            {
                if (string.IsNullOrWhiteSpace(line[0]) || line.Any(string.IsNullOrWhiteSpace)) continue;
                ...
                {
                    PercentageTotalManureProducedInSystem[animalType][manureStateType] =
                        double.Parse(line[column], InfrastructureConstants.EnglishCultureInfo);

                    column++;
                }

                var excretionRateForAnimal = double.Parse(line[5].ParseUntilOrDefault(),
                    InfrastructureConstants.EnglishCultureInfo);
                _excretionRates.Add(new Table_29_Default_Manure_Excreted_Provider_Data
                    { AnimalType = animalType, ManureExcretionRate = excretionRateForAnimal });
            }
        }
        ...
        {
            var animalTypeLookup = animalType;
            ...
            if (result != null) return result.ManureExcretionRate;
        ...
        public double GetPercentageOfManureProducedInSystem(AnimalType animalType, ManureStateType manureStateType)
        {
            ...
            if (animalType.IsBeefCattleType())
                ...
                animalTypeLookup = AnimalType.Sheep;
            else if (animalType.IsSwineType()) animalTypeLookup = AnimalType.Swine;

            if (PercentageTotalManureProducedInSystem.ContainsKey(animalTypeLookup))
                if (PercentageTotalManureProducedInSystem[animalTypeLookup].ContainsKey(manureStateLookup))
                    return PercentageTotalManureProducedInSystem[animalTypeLookup][manureStateLookup];

            Trace.TraceError(
                $"{nameof(Table_29_Default_Manure_Excreted_Provider)}.{nameof(GetPercentageOfManureProducedInSystem)}" +
        ...
        public class Table_29_Default_Manure_Excreted_Provider_Data
        {
            ...
            {
                return
                    $"{nameof(ManureExcretionRate)}: {ManureExcretionRate}, {nameof(ManureStateType)}: {ManureStateType}, {nameof(AnimalType)}: {AnimalType}";
            }
        ...
        private readonly List<Table_29_Default_Manure_Excreted_Provider_Data> _excretionRates =
            new List<Table_29_Default_Manure_Excreted_Provider_Data>();
        ...
        private readonly AnimalTypeStringConverter _animalTypeStringConverter = new AnimalTypeStringConverter();
        ...
        public MultiKeyDictionary<AnimalType, ManureStateType, double> PercentageTotalManureProducedInSystem { get; } =
            new MultiKeyDictionary<AnimalType, ManureStateType, double>();
