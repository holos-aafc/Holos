...
{
    /// <summary>
    ///     Table 40. Default values for PRgain (Protein retained in the sow (kg head-1 day-1), calculated according to Eq.
    ///     4.2.1 21)
    ///     by growth stage for growing pigs.
    ///     <para>1 Source: IPCC (2019), Table 10.20b</para>
    ///     <para>PRgain = Ngain * 6.25 </para>
    /// </summary>
    public class Table_40_Swine_Default_PrGain_Values_Provider
    {
        /// <summary>
        ...
        public double GetNitrogenRequiredForGain(double averageWeightOfAnimal)
        {
            ...
            if (averageWeightOfAnimal >= 4 && averageWeightOfAnimal < 7)
                // Nursery
                return 0.031;

            if (averageWeightOfAnimal >= 7 && averageWeightOfAnimal < 20)
                // Nursery
                return 0.028;

            if (averageWeightOfAnimal >= 20 && averageWeightOfAnimal < 40)
                // Grower
                return 0.025;

            if (averageWeightOfAnimal >= 40 && averageWeightOfAnimal < 80)
                // Grower
                return 0.024;

            if (averageWeightOfAnimal >= 80)
                // Grower
                return 0.021;
