...
namespace H.Core.Providers.Animals
{
    ...
    public class Table_6_Manure_Types_Default_Composition_Provider
    {
        ...
        public Table_6_Manure_Types_Default_Composition_Provider()
        {
            ...
            ManureCompositionData = new List<DefaultManureCompositionData>();
        ...
        public DefaultManureCompositionData GetManureCompositionDataByType(AnimalType animalType,
            ManureStateType manureStateType)
        {
            ...
            if (data != null)
                Trace.TraceError(
                    $"{nameof(Table_6_Manure_Types_Default_Composition_Provider)}.{nameof(GetManureCompositionDataByType)} " +
        ...
        private List<DefaultManureCompositionData> ReadFile()
        {
            ...
            foreach (var line in fileLines.Skip(1))
            {
                ...
                var animalType = _animalTypeStringConverter.Convert(line[0].ParseUntilOrDefault());
                var manureStateType = _manureStateTypeStringConverter.Convert(line[1].ParseUntilOrDefault());
                var moistureContent =
                    double.TryParse(line[2].ParseUntilOrDefault(), numberStyle, cultureInfo, out cellValue)
                        ? cellValue
                        : CoreConstants.ValueNotDetermined;
                var nfraction = double.TryParse(line[3].ParseUntilOrDefault(), numberStyle, cultureInfo, out cellValue)
                    ? cellValue
                    : CoreConstants.ValueNotDetermined;
                var cfraction = double.TryParse(line[4].ParseUntilOrDefault(), numberStyle, cultureInfo, out cellValue)
                    ? cellValue
                    : CoreConstants.ValueNotDetermined;
                var pfraction = double.TryParse(line[5].ParseUntilOrDefault(), numberStyle, cultureInfo, out cellValue)
                    ? cellValue
                    : CoreConstants.ValueNotDetermined;
                var cToNRatio = double.TryParse(line[6].ParseUntilOrDefault(), numberStyle, cultureInfo, out cellValue)
                    ? cellValue
                    : CoreConstants.ValueNotDetermined;
                var volatileSolidsContent =
                    double.TryParse(line[7].ParseUntilOrDefault(), numberStyle, cultureInfo, out cellValue)
                        ? cellValue
                        : CoreConstants.ValueNotDetermined;
