...
namespace H.Core.Providers.Carbon
{
    ...
    public class Table_7_Relative_Biomass_Information_Data
    {
        ...
        {
            return
                $"{nameof(CropType)}: {CropType}, {nameof(MoistureContentOfProduct)}: {MoistureContentOfProduct}, {nameof(RelativeBiomassProduct)}: {RelativeBiomassProduct}, {nameof(RelativeBiomassStraw)}: {RelativeBiomassStraw}, {nameof(RelativeBiomassRoot)}: {RelativeBiomassRoot}, {nameof(RelativeBiomassExtraroot)}: {RelativeBiomassExtraroot}";
        }
        ...
        public Dictionary<Province, Dictionary<SoilFunctionalCategory, double>>
            NitrogenFertilizerRateTable { get; set; } =
            new Dictionary<Province, Dictionary<SoilFunctionalCategory, double>>();

        public Dictionary<Province, Dictionary<SoilFunctionalCategory, double>>
            PhosphorusFertilizerRateTable { get; set; } =
            new Dictionary<Province, Dictionary<SoilFunctionalCategory, double>>();

        public Dictionary<Province, TillageType> TillageTypeTable { get; set; } =
            new Dictionary<Province, TillageType>();

        public Province? Province { get; set; }

        public Table_46_Biogas_Methane_Production_CropResidue_Data BiomethaneData { get; set; } =
            new Table_46_Biogas_Methane_Production_CropResidue_Data();
