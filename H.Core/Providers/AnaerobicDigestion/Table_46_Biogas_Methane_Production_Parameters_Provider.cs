...
namespace H.Core.Providers.AnaerobicDigestion
{
    ...
    public class Table_46_Biogas_Methane_Production_Parameters_Provider
    {
        ...
        /// <summary>
        ///     Reads the csv file and stores data into two lists. Each list corresponds to Manure Type and Farm Residue Type
        ///     substrates entries in the csv respectively.
        /// </summary>
        private void ReadFile()
        {
            ...
            var manureData = new List<Table_46_Biogas_Methane_Production_Manure_Data>();
            var residueData = new List<Table_46_Biogas_Methane_Production_FarmResidue_Data>();

            var cultureInfo = InfrastructureConstants.EnglishCultureInfo;
            var fileLines = CsvResourceReader.GetFileLines(CsvResourceNames.ParametersBiogasMethaneProduction);
            ...
            for (var row = 2; row < LastManureRow; row++)
            {
                var line = fileLines.ElementAt(row);

                var animalType = _animalTypeStringConverter.Convert(line[0]);
                var beddingType = _beddingMaterialTypeStringConverter.Convert(line[1]);
            ...
            foreach (var line in fileLines.Skip(11).Take(6))
            {
                var residueType = _farmResidueTypeStringConverter.Convert(line[0]);
                biomethanePotential = double.Parse(line[2], cultureInfo);
                methaneFraction = double.Parse(line[3], cultureInfo);
                volatileSolids = double.Parse(line[4], cultureInfo);
                totalSolids = double.Parse(line[5], cultureInfo);
        ...
        /// <returns>
        ///     Returns a single instance of <see cref="BiogasAndMethaneProductionParametersData" /> based on the parameters
        ///     specified.
        /// </returns>
        public BiogasAndMethaneProductionParametersData GetBiogasMethaneProductionInstance(AnimalType animalType,
            BeddingMaterialType beddingMaterial)
        {
            ...
            BiogasAndMethaneProductionParametersData data =
                ManureData.Find(x => x.AnimalType == lookupAnimalType && x.BeddingType == beddingMaterial);

            if (data != null) return data;

            data = ManureData.Find(x => x.AnimalType == lookupAnimalType);

            if (data != null) return data;

            Trace.TraceError(
                $"{nameof(Table_46_Biogas_Methane_Production_Parameters_Provider)}.{nameof(GetBiogasMethaneProductionInstance)}" +
        ...
        /// <returns>
        ///     Returns a single instance of <see cref="BiogasAndMethaneProductionParametersData" /> based on the parameters
        ///     specified. Returns an empty instance otherwise.
        /// </returns>
        public BiogasAndMethaneProductionParametersData GetBiogasMethaneProductionInstance(FarmResidueType residueType)
        {
            BiogasAndMethaneProductionParametersData data = FarmResiduesData.Find(x => x.ResidueType == residueType);
