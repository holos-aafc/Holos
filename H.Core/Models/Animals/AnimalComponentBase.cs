...
namespace H.Core.Models.Animals
{
    ...
    public abstract class AnimalComponentBase : ComponentBase
    {
        ...
        {
            var youngAnimalGroups = Groups.Where(x => x.GroupType == youngAnimalTypeAndStage.Item1);
            ...
            foreach (var youngAnimalGroup in youngAnimalGroups)
                totalYoungAnimals += youngAnimalGroup.GetNumberOfAnimalsInFirstManagementPeriodByProductionStage(
                    youngAnimalTypeAndStage.Item2);

            var totalOtherAnimals = 0d;
            foreach (var animalType in otherAnimalsAndStages)
            {
                var otherAnimalGroups = Groups.Where(x => x.GroupType == animalType.Item1);
            ...
            if (totalOtherAnimals <= 0) return 0;

            return totalYoungAnimals / totalOtherAnimals;
        }

        /// <summary>
        ///     Searches through the component's group of animals looking for the group of animals that has the same pairing number
        ///     as that of the given young animal group.
        /// </summary>
        ...
        /// <returns>
        ///     A single <see cref="AnimalGroup" /> that is the parent of the specified young animal group or null if no
        ///     associated parent group was found
        /// </returns>
            ...
            AnimalType parentGroupType)
        {
            if (parentGroupType.IsLactatingType() == false)
            {
                Trace.TraceInformation(
                    $"{nameof(AnimalComponentBase)}.{nameof(GetAssociatedParentGroup)}: parent group type '{parentGroupType.GetDescription()}' is not a lactating animal group type, so there can be no associate parent group for this group of young animals: '{youngAnimalGroup.GroupType.GetDescription()}'.");
            ...
            if (youngAnimalGroup.GroupPairingNumber > 0)
                return Groups.FirstOrDefault(group =>
                    ...
                    group.GroupPairingNumber.Equals(youngAnimalGroup.GroupPairingNumber));

            // Orphaned group, assign to first parent of appropriate type
            var firstParent = Groups.FirstOrDefault(group => group.GroupType == parentGroupType);
            return firstParent;
        }

        /// <summary>
        ///     Returns the <see cref="ManagementPeriod" /> of the parent group of animals for a given <see cref="AnimalGroup" />
        ///     of young animals on a given <see cref="DateTime" />.
        ///     It is possible to have null returned when there is no <see cref="ManagementPeriod" /> belonging to the parent
        ///     <see cref="AnimalGroup" /> that contains the specified date.
        /// </summary>
        ...
        public ManagementPeriod GetAssociatedManagementPeriodOfParentGroup(
            ...
            AnimalType parentGroupType)
        {
            var parentAnimalGroup = GetAssociatedParentGroup(
                youngAnimalGroup,
                parentGroupType);

            if (parentAnimalGroup != null) return parentAnimalGroup.GetManagementPeriodByDate(dateTime);

            return null;
        }

        /// <summary>
        ///     Searches through the component's group of animals looking for animal groups that have the same pairing number as
        ///     the parent group type (i.e. searches for all
        ///     calf groups that are associated with a group of cows). The parent <see cref="AnimalGroup" /> must be a lactating
        ///     type of animal otherwise no associated
            ...
            AnimalType childGroupType)
        {
            ...
            if (parentGroup.GroupType.IsLactatingType() == false) return result;
            ...
            {
                var groups = Groups.Where(group =>
                    group.GroupType == childGroupType &&
                    group.GroupPairingNumber.Equals(parentGroup.GroupPairingNumber)).ToList();
                if (groups.Any()) result.AddRange(groups);
            }
        ...
        /// <summary>
        ///     Searches through the list of groups and finds all <see cref="ManagementPeriod" /> for the given
        ///     <see cref="AnimalType" /> and the specified <see cref="DateTime" />.
        /// </summary>
        public List<ManagementPeriod> GetAssociateManagementPeriodsByGroup(AnimalType animalType, DateTime dateTime)
        {
            ...
            var animalGroups = Groups.Where(x => x.GroupType == animalType);
            foreach (var animalGroup in animalGroups)
            {
                var managementPeriod = animalGroup.GetManagementPeriodByDate(dateTime);
                if (managementPeriod != null) result.Add(managementPeriod);
            }
        ...
        /// <summary>
        ///     Gets the total number of young animals associated with a parent <see cref="AnimalGroup" /> by
        ///     <see cref="DateTime" />.
        /// </summary>
        /// <param name="dateTime">The <see cref="DateTime" /> to search by</param>
        ...
        public int GetTotalNumberOfYoungAnimalsByDate(DateTime dateTime, AnimalGroup parentGroup,
            AnimalType childGroupType)
        {
            var result = 0;
            ...
            var associatedYoungAnimalGroups = GetAssociatedYoungAnimalGroups(
                parentGroup,
                childGroupType);
        ...
        public double GetFertilityRate(AnimalType targetAnimalType, IEnumerable<ProductionStages> productionStages)
        {
            ...
            foreach (var animalGroup in Groups.Where(x => x.GroupType == targetAnimalType))
            {
                var totalDaysInAllProductionStages = 0.0;
                foreach (var productionStage in productionStages)
                {
                    var totalDaysInThisProductionStage = animalGroup.ManagementPeriods
                        .Where(x => x.ProductionStage == productionStage).Sum(x => x.Duration.TotalDays);
        ...
        private void GroupsOnCollectionChanged(object sender,
