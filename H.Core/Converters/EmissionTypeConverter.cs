...
namespace H.Core.Converters
{
    public class EmissionTypeConverter
    {
        ...
        public double Convert(EmissionDisplayUnits sourceType, EmissionDisplayUnits targetType, double emissionValue)
        {
            ...
            {
                if (targetType == EmissionDisplayUnits.MegagramsCO2e) return emissionValue / 1000;

                // No conversion needed
                if (targetType == EmissionDisplayUnits.KilogramsC02e) return emissionValue;
            }

            throw new Exception(
                $"Unknown conversion for source type: '{sourceType.GetDescription()}' to target type: '{targetType.GetDescription()}'");
        }
