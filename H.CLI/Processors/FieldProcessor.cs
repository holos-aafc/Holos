...
namespace H.CLI.Processors
{
    public class FieldProcessor : IProcessor
    {
        ...
        public void ProcessComponent(Farm farm, List<ComponentBase> componentList, ApplicationData applicationData)
        {
             ...
             *
             * Worth knowing what that costs, because it decides whether a change to the carbon or nitrogen pipeline
             * reaches the CLI at all. CreateDetailViewItems is never called on this path, so nothing living inside it
             * runs here: the perennial adjustments (ProcessPerennials, AssignPerennialRootsReturned,
             * UpdateYieldFromHarvest, UpdatePercentageReturnsForPerennials, PostProcessPerennials) and the grazing
             * carbon and manure-on-pasture calls. The input file supplies those values instead.
             *
             * The carbon input calculators DO run, through CarbonService.ProcessCommandLineItems - but only when the
             * row leaves both residue carbon columns at zero. A file exported from the interface carries non-zero
             * values, so the file wins and nothing is recomputed. That is why an input file generated by an earlier
             * version keeps that version's carbon values after an upgrade, which the User Guide explains to users.
             */
