...
public class CropEconomicProviderTest
{
    ...
    {
        var notSelected = _provider.HasDataForCropType(CropType.NotSelected);
        Assert.IsFalse(notSelected);

        var fieldPeas = _provider.HasDataForCropType(CropType.FieldPeas);
        Assert.IsTrue(fieldPeas);

        var oatSilage = _provider.HasDataForCropType(CropType.OatSilage);
    ...
    {
        var alberta = _provider.HasDataForProvince(Province.Alberta);
        Assert.IsTrue(alberta);

        var bc = _provider.HasDataForProvince(Province.BritishColumbia);
        Assert.IsFalse(bc);
    }
    ...
    {
        var saskSuccess = _provider.FarmCropTypeToEconCropType(CropType.CanarySeed, SoilFunctionalCategory.Brown,
            Province.Saskatchewan);
        Assert.AreEqual(CropType.CanarySeed, saskSuccess);

        var manSuccess =
            _provider.FarmCropTypeToEconCropType(CropType.Oats, SoilFunctionalCategory.NotApplicable,
                Province.Manitoba);
        Assert.AreEqual(CropType.Oats, manSuccess);

        var abSuccess =
            _provider.FarmCropTypeToEconCropType(CropType.OatSilage, SoilFunctionalCategory.Brown, Province.Alberta);
        Assert.AreEqual(CropType.CerealSilage, abSuccess);

        var abNotSelected =
            _provider.FarmCropTypeToEconCropType(CropType.Potatoes, SoilFunctionalCategory.Brown, Province.Alberta);
        Assert.AreEqual(CropType.NotSelected, abNotSelected);

        var saskNotSelected =
            _provider.FarmCropTypeToEconCropType(CropType.Potatoes, SoilFunctionalCategory.Brown,
                Province.Saskatchewan);
        Assert.AreEqual(CropType.NotSelected, saskNotSelected);

        var manNotSelected = _provider.FarmCropTypeToEconCropType(CropType.Potatoes,
            SoilFunctionalCategory.NotApplicable, Province.Manitoba);
        Assert.AreEqual(CropType.NotSelected, manNotSelected);

        var onNotSelected = _provider.FarmCropTypeToEconCropType(CropType.Potatoes,
            SoilFunctionalCategory.NotApplicable, Province.Ontario);
    ...
    {
        var result = _provider.GetCropEconomicDataByProvinceAndSoilZone(Province.Alberta, SoilFunctionalCategory.Brown)
            .ToList();
    ...
    {
        var brown = _provider.GetCropEconomicDataByProvinceAndSoilZone(Province.Saskatchewan,
            SoilFunctionalCategory.Brown);

        var darkBrown =
            _provider.GetCropEconomicDataByProvinceAndSoilZone(Province.Saskatchewan, SoilFunctionalCategory.DarkBrown);

        var black = _provider.GetCropEconomicDataByProvinceAndSoilZone(Province.Saskatchewan,
