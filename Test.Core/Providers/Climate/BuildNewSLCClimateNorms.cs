...
{
    private readonly string outPutDirectory = @"C:\Users\bigbe\Holos\slcDaily\2021_new_climate_normals";

    /// <summary>
    ///     Where the csv files with the SLC data exists
    /// </summary>
    private readonly string workingDirectory = @"C:\Users\bigbe\Holos\slcDaily\additionalSLC";
    ...
    public void BuildUpdatedLethbridgeClimateNorms()
    {
        ...
        var customTimeFrameStart = 1980;
        var customTimeFrameEnd = 2010;
        var polygonId = 793011;
        ...
        var temperatureNormals =
            climateNormalCalculator.GetTemperatureDataByDailyValues(listOfDailyClimateData, customTimeFrameStart,
                customTimeFrameEnd);
        var precipiationNormals =
            climateNormalCalculator.GetPrecipitationDataByDailyValues(listOfDailyClimateData, customTimeFrameStart,
                customTimeFrameEnd);
        var evapotranspirationNormals =
            climateNormalCalculator.GetEvapotranspirationDataByDailyValues(listOfDailyClimateData, customTimeFrameStart,
                customTimeFrameEnd);
        ...
        {
            var line =
                $"{polygonId},{(int)month},{temperatureNormals.GetMeanTemperatureForMonth((int)month)},{precipiationNormals.GetValueByMonth(month)},{evapotranspirationNormals.GetValueByMonth(month)}";
            lines.Add(line);
        }
    ...
    public void BuildUpdatedClimateNormsByPolygon()
    {
        ...
        for (var i = 1950; i <= 1990; i += 10)
        {
            ...
            foreach (var fileString in allFileStrings)
            {
                ...
                var temperatureNormals = climateNormalCalculator.GetTemperatureDataByDailyValues(listOfDailyClimateData,
                    customTimeFrameStart, customTimeFrameEnd);
                var precipiationNormals =
                    climateNormalCalculator.GetPrecipitationDataByDailyValues(listOfDailyClimateData,
                        customTimeFrameStart, customTimeFrameEnd);
                var evapotranspirationNormals =
                    climateNormalCalculator.GetEvapotranspirationDataByDailyValues(listOfDailyClimateData,
                        customTimeFrameStart, customTimeFrameEnd);
