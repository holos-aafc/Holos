...
public class SlcClimateNormalsDataProviderTest
{
    ...
    private static readonly string header = "month,old,new,difference";
    ...
    public void CheckGetPrecipitationDataByPolygonReadsCorrectFile()
    {
        ...
        var precip1990data =
            _sut.GetPrecipitationDataByPolygonId(polygonId1, TimeFrame.NineteenNinetyToTwoThousandSeventeen);
    ...
    public void CheckGetEvapotranspirationByPolygonReadsCorrectFile()
    {
        ...
        var PET1950data =
            _sut.GetEvapotranspirationDataByPolygonId(polygonId1, TimeFrame.NineteenFiftyToNineteenEighty);
        var PET1960data =
            _sut.GetEvapotranspirationDataByPolygonId(polygonId1, TimeFrame.NineteenSixtyToNineteenNinety);
        var PET1970data = _sut.GetEvapotranspirationDataByPolygonId(polygonId1, TimeFrame.NineteenSeventyToTwoThousand);
        var PET1980data =
            _sut.GetEvapotranspirationDataByPolygonId(polygonId1, TimeFrame.NineteenEightyToTwoThousandTen);
        var PET1990data =
            _sut.GetEvapotranspirationDataByPolygonId(polygonId1, TimeFrame.NineteenNinetyToTwoThousandSeventeen);
    ...
    public void CheckGetTemperatureByPolgyonReadsCorrectFileTest()
    {
        ...
        var temp1990data =
            _sut.GetTemperatureDataByPolygonId(polygonId1, TimeFrame.NineteenNinetyToTwoThousandSeventeen);
    ...
    public void CompareOldTemperatureDataToNewTemperatureData()
    {
        ...
        var newTemperatureData =
            _sut.GetTemperatureDataByPolygonId(lethbridgePolygon, TimeFrame.NineteenEightyToTwoThousandTen);
    ...
    public void CompareOldPrecipitationDataToNewPrecipitationData()
    {
        var lethbridgePolygon = 793011;
        // get the data from the new climate norms
        var newPrecipitationData =
            _sut.GetPrecipitationDataByPolygonId(lethbridgePolygon, TimeFrame.NineteenEightyToTwoThousandTen);
    ...
    public void CompareOldPETwithNewPET()
    {
        var lethbridgePolygon = 793011;
        // get the data from the new climate norms
        var newEvapotranspirationData =
            _sut.GetEvapotranspirationDataByPolygonId(lethbridgePolygon, TimeFrame.NineteenEightyToTwoThousandTen);

        //get the data from the old climate by passing in an unhandled timeframe
        var oldEvapotranspirationData =
            _sut.GetEvapotranspirationDataByPolygonId(lethbridgePolygon, TimeFrame.ProjectionPeriod);
        ...
        {
            var newMonth = (double)newEvapotranspirationData.GetType().GetProperty(month)
                .GetValue(newEvapotranspirationData);
            var oldMonth = (double)oldEvapotranspirationData.GetType().GetProperty(month)
                .GetValue(oldEvapotranspirationData);
            var result = oldMonth - newMonth;
            lines.Add($"{month},{oldMonth},{newMonth},{result}");
            Assert.AreNotEqual(newMonth, oldMonth);
        }
    ...
    public void CompareDailyTemperatureFromNasaVsSLC()
    {
        ...
        for (var i = 1; i <= 365; i++)
        {
            var slcMeanTemp = slcLethbridge1990DailyData[i - 1].MeanDailyAirTemperature;
            var nasaMeanTemp = nasaLethbridge1990DailyData[i - 1].MeanDailyAirTemperature;
            var difference = slcMeanTemp - nasaMeanTemp;
    ...
    public void CompareDailyPrecipFromNasaVsSLC()
    {
        ...
        for (var i = 1; i <= 365; i++)
        {
            var slcMeanPrecip = slcLethbridge1990DailyData[i - 1].MeanDailyPrecipitation;
            var nasaMeanPrecip = nasaLethbridge1990DailyData[i - 1].MeanDailyPrecipitation;
            var difference = slcMeanPrecip - nasaMeanPrecip;
    ...
    public void CompareDailyPETFromNasaVsSLC()
    {
        ...
        for (var i = 1; i <= 365; i++)
        {
            var slcMeanPET = slcLethbridge1990DailyData[i - 1].MeanDailyPET;
            var nasaMeanPET = nasaLethbridge1990DailyData[i - 1].MeanDailyPET;
            var difference = slcMeanPET - nasaMeanPET;
    ...
    private Tuple<List<DailyClimateData>, List<DailyClimateData>> GetDailyClimateDataForComparisonSlcNasa(
        double lethbridgeLong, double lethbridgeLat)
    {
        ...
        var slcAndNasaTuple =
            new Tuple<List<DailyClimateData>, List<DailyClimateData>>(slcLethbridge1990DailyData,
                nasaLethbridge1990DailyData);
