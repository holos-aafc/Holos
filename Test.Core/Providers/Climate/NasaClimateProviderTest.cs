...
public class NasaClimateProviderTest
{
    ...
    public void GetGrowingSeasonPrecipitationFromOntarioReturnsMoreThan400()
    {
        ...
        if (northData.Any() == false || eastData.Any() == false || southData.Any() == false ||
            westData.Any() == false || centralData.Any() == false) return;
        ...
        // present data
        var northGrowingSeasonValues = northData.Where(x => x.Year == 2019 && x.JulianDay >= 122 && x.JulianDay <= 305)
            .Select(x => x.MeanDailyPrecipitation);
        var eastGrowingSeasonValues = eastData.Where(x => x.Year == 2019 && x.JulianDay >= 122 && x.JulianDay <= 305)
            .Select(x => x.MeanDailyPrecipitation);
        var southGrowingSeasonValues = southData.Where(x => x.Year == 2019 && x.JulianDay >= 122 && x.JulianDay <= 305)
            .Select(x => x.MeanDailyPrecipitation);
        var westGrowingSeasonValues = westData.Where(x => x.Year == 2019 && x.JulianDay >= 122 && x.JulianDay <= 305)
            .Select(x => x.MeanDailyPrecipitation);
        var centralGrowingSeasonValues = centralData
            .Where(x => x.Year == 2019 && x.JulianDay >= 122 && x.JulianDay <= 305)
            .Select(x => x.MeanDailyPrecipitation);
        ...
        // 1985 data
        var northGrowingSeasonValues85 = northData
            .Where(x => x.Year == 1985 && x.JulianDay >= 122 && x.JulianDay <= 305)
            .Select(x => x.MeanDailyPrecipitation);
        var eastGrowingSeasonValues85 = eastData.Where(x => x.Year == 1985 && x.JulianDay >= 122 && x.JulianDay <= 305)
            .Select(x => x.MeanDailyPrecipitation);
        var southGrowingSeasonValues85 = southData
            .Where(x => x.Year == 1985 && x.JulianDay >= 122 && x.JulianDay <= 305)
            .Select(x => x.MeanDailyPrecipitation);
        var westGrowingSeasonValues85 = westData.Where(x => x.Year == 1985 && x.JulianDay >= 122 && x.JulianDay <= 305)
            .Select(x => x.MeanDailyPrecipitation);
        var centralGrowingSeasonValues85 = centralData
            .Where(x => x.Year == 1985 && x.JulianDay >= 122 && x.JulianDay <= 305)
            .Select(x => x.MeanDailyPrecipitation);
    ...
    public void GetClimateData()
    {
        var lines = CsvResourceReader.SplitFileIntoLinesUsingRegex(
            File.ReadAllText("Climate\\Missing Climate Data_perennials.csv"));
