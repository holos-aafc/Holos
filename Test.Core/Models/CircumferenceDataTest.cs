...
public class CircumferenceDataTest
{
    ...
    public void CopyConstructorDoesDeepCopy()
    {
        ...
        //Identical
        if (!DeepEqual(a.Table, b.Table)) Assert.Fail();

        if (!DeepEqual(a.ValidRows, b.ValidRows)) Assert.Fail();

        if (!DeepEqual(a.QuickAccessRows, b.QuickAccessRows)) Assert.Fail();

        if (!DeepEqual(a.InvalidRows, b.InvalidRows)) Assert.Fail();

        if (!DeepEqual(a.ValidColumns, b.ValidColumns)) Assert.Fail();

        if (!DeepEqual(a.QuickAccessColumns, b.QuickAccessColumns)) Assert.Fail();

        if (!DeepEqual(a.InvalidColumns, b.InvalidColumns)) Assert.Fail();

        if (!DeepEqual(a.UserCircumference, b.UserCircumference)) Assert.Fail();

        if (!DeepEqual(a.CircumferenceGenerationOverriden, b.CircumferenceGenerationOverriden)) Assert.Fail();
    }
    ...
    public void GetTreeIndicesReturnsCorrectTreeIndice()
    {
        ...
        for (var i = 0; i < 10; ++i) trees.Add(data.NewTree());
