...
public abstract class UnitTestBase
{
    ...
    public Farm GetTestFarm()
    {
        ...
        {
            DateOfExport = DateTime.Now, Amount = 1000, AnimalType = AnimalType.Dairy,
            DefaultManureCompositionData = new DefaultManureCompositionData { NitrogenContent = 0.5 }
        });
        farm.ManureExportViewItems.Add(new ManureExportViewItem
        {
            DateOfExport = DateTime.Now, Amount = 2000, AnimalType = AnimalType.Dairy,
            DefaultManureCompositionData = new DefaultManureCompositionData { NitrogenContent = 0.5 }
        });
    ...
    public AnimalComponentEmissionsResults GetEmptyTestAnimalComponentEmissionsResults()
    {
        ...
        var groupEmissionsByMonth =
            new GroupEmissionsByMonth(monthsAndDaysData, new List<GroupEmissionsByDay> { groupEmissionsByDay });
        ...
        results.EmissionResultsForAllAnimalGroupsInComponent =
            new List<AnimalGroupEmissionResults> { animalGroupResults };
    ...
    public AnimalComponentEmissionsResults GetNonEmptyTestBeefCattleAnimalComponentEmissionsResults()
    {
        ...
        var groupEmissionsByMonth = new GroupEmissionsByMonth(monthsAndDaysData,
            new List<GroupEmissionsByDay> { GetGroupEmissionsByDay() });
    ...
    public AnimalComponentEmissionsResults GetTestGrazingBeefCattleAnimalComponentEmissionsResults(
        FieldSystemComponent fieldSystemComponent)
    {
        var results = new AnimalComponentEmissionsResults();
        results.Component = GetTestGrazingAnimalComponent(fieldSystemComponent);

        var animalGroupResults = GetTestGrazingBeefCattleAnimalGroupComponentEmissionsResults(fieldSystemComponent);
    ...
    public AnimalGroupEmissionResults GetTestGrazingBeefCattleAnimalGroupComponentEmissionsResults(
        FieldSystemComponent fieldSystemComponent)
    {
        var animalGroupResults = new AnimalGroupEmissionResults();

        var groupEmissionsByMonth = GetTestGroupEmissionsByMonthForGrazingAnimals(fieldSystemComponent);
        animalGroupResults.GroupEmissionsByMonths = new List<GroupEmissionsByMonth> { groupEmissionsByMonth };

        return animalGroupResults;
    }

    public GroupEmissionsByMonth GetTestGroupEmissionsByMonthForGrazingAnimals(
        FieldSystemComponent fieldSystemComponent)
    {
        var monthsAndDaysData = new MonthsAndDaysData();
        monthsAndDaysData.Year = DateTime.Now.Year;
    ...
    public GeographicData GetTestGeographicData()
    {
        ...
        geographicData.SoilDataForAllComponentsWithinPolygon.Add(GetTestSoilData());
