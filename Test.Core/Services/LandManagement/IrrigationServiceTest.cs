...
public class IrrigationServiceTest
{
    ...
    public void AddIrrigationToDailyPrecipitationsTest()
    {
        ...
        for (var i = 0; i < 365; i++) dailyPrecipitation.Add(10);
        ...
        foreach (var d in result)
        {
            ...
            var totalPrecipitation = _sut.GetTotalDailyPrecipitation(julianDay, precipitation, farm.Province, crop.Year,
                crop.AmountOfIrrigation);
            var expected = precipitation + crop.AmountOfIrrigation * (percentage / 100.0) /
                DateTime.DaysInMonth(crop.Year, (int)month);
