...
public class ClimateNormalCalculatorTest
{
    ...
    public void GetNormalsForAllTwelveMonthsReturnsCorrectValue()
    {
        ...
        if (!_dailyClimateData.Any()) return;
    ...
    public void GetTemperatureDataByDailyValueNotReturnNull()
    {
        ...
        var result = _calculator.GetTemperatureDataByDailyValues(_dailyClimateData, TimeFrame.TwoThousandToCurrent);
    ...
    public void GetPrecipitationDataByDailyValueNotReturnNull()
    {
        ...
        var result = _calculator.GetPrecipitationDataByDailyValues(_dailyClimateData, TimeFrame.TwoThousandToCurrent);
    ...
    public void GetGrowingSeasonPrecipitationNormalsForOntario()
    {
        ...
        // return early in these situations to prevent unhandled exceptions or errors that lead to a failed test
        if (!customClimateData.Any()) return;

        var normalsFor1990To2000 =
            _calculator.GetPrecipitationDataByDailyValues(customClimateData, TimeFrame.TwoThousandToCurrent);

        var growingSeasonPrecipitation1990To2000 = normalsFor1990To2000.CalculateGrowingSeasonPrecipitation();
        var growingSeasonUsingDailyValues = customClimateData
            .Where(x => x.Year == 2019 && x.JulianDay >= 122 && x.JulianDay <= 305)
            .Select(x => x.MeanDailyPrecipitation).Sum();

        var normalsFor1980To1990 =
            _calculator.GetPrecipitationDataByDailyValues(customClimateData, TimeFrame.TwoThousandToCurrent);
