...
public class IPCCTier2SoilCarbonCalculatorTest : UnitTestBase
{
    ...
    private readonly IPCCTier2CarbonInputCalculator _inputCalculator = new();
    ...
    public void TestInitialize()
    {
        ...
        lines = data.Split(new[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries);
        foreach (var line in lines.Skip(1))
        {
        ...
        _f1 = _globallyCalibratedModelParametersProvider
            .GetGloballyCalibratedModelParametersInstance(ModelParameters.FractionMetabolicDMActivePool, _tillageType)
            .Value;
        _f2 = _globallyCalibratedModelParametersProvider
            .GetGloballyCalibratedModelParametersInstance(ModelParameters.FractionStructuralDMActivePool, _tillageType)
            .Value;
        _f3 = _globallyCalibratedModelParametersProvider
            .GetGloballyCalibratedModelParametersInstance(ModelParameters.FractionStructuralDMSlowPool, _tillageType)
            .Value;
        _f5 = _globallyCalibratedModelParametersProvider
            .GetGloballyCalibratedModelParametersInstance(ModelParameters.FractionActiveDecayToPassive, _tillageType)
            .Value;
        _f6 = _globallyCalibratedModelParametersProvider
            .GetGloballyCalibratedModelParametersInstance(ModelParameters.FractionSlowDecayToPassive, _tillageType)
            .Value;
        _f7 = _globallyCalibratedModelParametersProvider
            .GetGloballyCalibratedModelParametersInstance(ModelParameters.FractionSlowDecayToActive, _tillageType)
            .Value;
        _f8 = _globallyCalibratedModelParametersProvider
            .GetGloballyCalibratedModelParametersInstance(ModelParameters.FractionPassiveDecayToActive, _tillageType)
            .Value;

        _activePoolDecayRateConstant = _globallyCalibratedModelParametersProvider
            .GetGloballyCalibratedModelParametersInstance(ModelParameters.DecayRateActive, _tillageType).Value;
        _slowPoolDecayRateConstant = _globallyCalibratedModelParametersProvider
            .GetGloballyCalibratedModelParametersInstance(ModelParameters.DecayRateSlow, _tillageType).Value;
        _passivePoolDecayRateConstant = _globallyCalibratedModelParametersProvider
            .GetGloballyCalibratedModelParametersInstance(ModelParameters.DecayRatePassive, _tillageType).Value;

        _tillageFactor = _globallyCalibratedModelParametersProvider
            .GetGloballyCalibratedModelParametersInstance(ModelParameters.TillageModifier, _tillageType).Value;
    }
    ...
    public void CalculateMonthlyTemperatureFactor()
    {
        ...
        {
            var optimumTemperature = _globallyCalibratedModelParametersProvider
                .GetGloballyCalibratedModelParametersInstance(
                    ModelParameters.OptimumTemperature,
                    _tillageType).Value;

            var maximumTemperature = _globallyCalibratedModelParametersProvider
                .GetGloballyCalibratedModelParametersInstance(
            ...
            for (var index = 0; index < monthsForYear.Count; index++)
            {
                ...
                var calculated =
                    _sut.CalculateMonthlyTemperatureEffectOnDecomposition(maximumTemperature, temperature,
                        optimumTemperature);
    ...
    public void CalculateWaterEffectForMonth()
    {
        ...
        foreach (var distinctYear in distinctYears)
        {
            ...
            for (var index = 0; index < monthsForYear.Count; index++)
            {
                ...
                var slopeParameter = _globallyCalibratedModelParametersProvider
                    .GetGloballyCalibratedModelParametersInstance(
                        ModelParameters.SlopeParameter,
                        TillageType.Intensive).Value;

                var calculated = _sut.CalculateMonthlyWaterEffectOnDecomposition(
                    precipitationForMonth,
                    evapotranspiration,
                    slopeParameter);
    ...
    public void CalculateAnnualTemperatureFactor()
    {
        foreach (var distinctYear in _annualTestData)
        {
            ...
            var maximumTemperatureForDecomposition = _globallyCalibratedModelParametersProvider
                .GetGloballyCalibratedModelParametersInstance(
                    ModelParameters.MaximumAvgTemperature,
                    TillageType.Intensive).Value;

            var optimumTemperatureForDecomposition = _globallyCalibratedModelParametersProvider
                .GetGloballyCalibratedModelParametersInstance(
                    ModelParameters.OptimumTemperature,
                    TillageType.Intensive).Value;

            var calculated = _sut.CalculateAverageAnnualTemperatureFactor(
                monthsForYear.Select(x => x.AverageTemperature).ToList(), maximumTemperatureForDecomposition,
    ...
    public void CalculateAnnualWaterFactor()
    {
        foreach (var distinctYear in _annualTestData)
        {
            ...
            var calculated = _sut.CalculateAnnualWaterFactor(monthsForYear.Select(x => x.Precipitation).ToList(),
                monthsForYear.Select(x => x.Evapotranspiration).ToList(), slopeParameter);
    ...
    public void CalculateBeta()
    {
        foreach (var year in _annualCarbonData)
        {
            var calculatedValue = _sut.CalculateAmountToDeadMatterComponent(
                year.TotalCarbonInputs,
                year.NitrogenContent,
                year.LigninContent);

            Assert.AreEqual(_annualCarbonData.Single(x => x.Year == year.Year).IpccTier2CarbonResults.Beta,
    ...
    public void CalculateSteadyStateForActiveSubPool()
    {
        foreach (var year in _annualCarbonData)
        {
            ...
            var calculateValue = _sut.CalculateSteadyStateActivePool(
                year.IpccTier2CarbonResults.Alpha,
                decayRate);
    ...
    public void CalculateActiveSubPool()
    {
        ...
        for (var i = 0; i < _annualCarbonData.Count; i++)
        {
            var carbonDataAtYear = _annualCarbonData.ElementAt(i);
            var previousYearResults =
                i == 0
                    ? _averageRunInValuesAndInitialStocks.IpccTier2CarbonResults.ActivePool
                    : calclatedResults.ElementAt(i - 1);

            var calculateValue = _sut.CalculateActivePoolAtCurrentInterval(
                previousYearResults,
                carbonDataAtYear.IpccTier2CarbonResults.ActivePoolSteadyState,
                carbonDataAtYear.IpccTier2CarbonResults.ActivePoolDecayRate);
    ...
    public void CalculateSteadyStateForSlowSubPool()
    {
        foreach (var year in _annualCarbonData)
        {
            ...
            var calculateValue = _sut.CalculateSteadyStateSlowPool(
                ...
                year.LigninContent,
                _f3,
                year.IpccTier2CarbonResults.ActivePoolSteadyState,
                year.IpccTier2CarbonResults.ActivePoolDecayRate,
                f4,
                year.IpccTier2CarbonResults.SlowPoolDecayRate);
    ...
    public void CalculateSlowSubPool()
    {
        ...
        for (var i = 0; i < _annualCarbonData.Count; i++)
        {
            ...
            var previousYearResults =
                i == 0
                    ? _averageRunInValuesAndInitialStocks.IpccTier2CarbonResults.SlowPool
                    : calclatedResults.ElementAt(i - 1);

            var calculateValue = _sut.CalculateSlowPoolAtInterval(
                previousYearResults,
                carbonDataAtYear.IpccTier2CarbonResults.SlowPoolSteadyState,
                carbonDataAtYear.IpccTier2CarbonResults.SlowPoolDecayRate);
    ...
    public void CalculateSteadyStateForPassiveSubPool()
    {
        foreach (var year in _annualCarbonData)
        {
            var calculateValue = _sut.CalculatePassivePoolSteadyState(
                ...
                year.IpccTier2CarbonResults.ActivePoolDecayRate,
                _f5,
                year.IpccTier2CarbonResults.SlowPoolSteadyState,
                year.IpccTier2CarbonResults.SlowPoolDecayRate,
                _f6,
                year.IpccTier2CarbonResults.PassivePoolDecayRate);
    ...
    public void CalculatePassiveSubPool()
    {
        ...
        for (var i = 0; i < _annualCarbonData.Count; i++)
        {
            ...
            var previousYearResults =
                i == 0
                    ? _averageRunInValuesAndInitialStocks.IpccTier2CarbonResults.PassivePool
                    : calclatedResults.ElementAt(i - 1);

            var calculateValue = _sut.CalculatePassivePoolAtInterval(
                previousYearResults,
                carbonDataAtYear.IpccTier2CarbonResults.PassivePoolSteadyState,
                carbonDataAtYear.IpccTier2CarbonResults.PassivePoolDecayRate);
    ...
    public void CalculateSoc()
    {
        for (var i = 0; i < _annualCarbonData.Count; i++)
        {
            ...
            var calculatedValue = _sut.CalculateTotalStocks(
                carbonDataAtYear.IpccTier2CarbonResults.ActivePool,
                carbonDataAtYear.IpccTier2CarbonResults.PassivePool,
                carbonDataAtYear.IpccTier2CarbonResults.SlowPool);
    ...
    public void CalculateSocDiff()
    {
        ...
        for (var i = 0; i < _annualCarbonData.Count; i++)
        {
            var carbonDataAtYear = _annualCarbonData.ElementAt(i);
            var previousYearResults =
                i == 0 ? _averageRunInValuesAndInitialStocks.Soc : calclatedResults.ElementAt(i - 1);

            var calculateValue = _sut.CalculateStockChange(
                carbonDataAtYear.Soc,
                previousYearResults);
    ...
    public void CalculateResults()
    {
        ...
        var viewItems = new List<CropViewItem>
        {
            new()
            {
                Year = 1985, CropType = CropType.Barley, Yield = 5, LigninContent = 0.1, NitrogenContent = 0.3,
                AboveGroundResidueDryMatter = 20, NitrogenContentInStraw = 0.8, NitrogenContentInRoots = 0.7,
                PercentageOfStrawReturnedToSoil = 100, BiomassCoefficientStraw = 0.2,
                BiomassCoefficientProduct = 0.6, FieldSystemComponentGuid = fieldSystemComponent.Guid
            },
            new()
            {
                Year = 1986, CropType = CropType.Barley, Yield = 2, LigninContent = 0.1, NitrogenContent = 0.3,
                AboveGroundNitrogenResidueForCrop = 20, NitrogenContentInStraw = 0.8, NitrogenContentInRoots = 0.7,
                PercentageOfStrawReturnedToSoil = 100, BiomassCoefficientStraw = 0.2,
                BiomassCoefficientProduct = 0.6, FieldSystemComponentGuid = fieldSystemComponent.Guid
            },
            new()
            {
                Year = 1987, CropType = CropType.Barley, Yield = 3, LigninContent = 0.1, NitrogenContent = 0.3,
                AboveGroundResidueDryMatter = 20, NitrogenContentInStraw = 0.8, NitrogenContentInRoots = 0.7,
                PercentageOfStrawReturnedToSoil = 100, BiomassCoefficientStraw = 0.2,
                BiomassCoefficientProduct = 0.6, FieldSystemComponentGuid = fieldSystemComponent.Guid
            }
    ...
    public void UseCustomStartingPoint()
    {
        ...
        var runInPeriodItems = new List<CropViewItem>
        {
            new()
            {
                CropType = CropType.Wheat, TotalCarbonInputs = 1000,
                FieldSystemComponentGuid = fieldSystemComponent.Guid
            },
            new()
            {
                CropType = CropType.Oats, TotalCarbonInputs = 5000, FieldSystemComponentGuid = fieldSystemComponent.Guid
            },
            new()
            {
                CropType = CropType.Barley, TotalCarbonInputs = 200,
                FieldSystemComponentGuid = fieldSystemComponent.Guid
            }
        };
        ...
        Assert.AreEqual(farm.StartingSoilOrganicCarbon,
            customStartingActivePool + customStartingPassivePool + customStartingSlowPool);
        ...
        {
            new() { Year = 1985, FieldSystemComponentGuid = fieldSystemComponent.Guid },
            new() { Year = 1986, FieldSystemComponentGuid = fieldSystemComponent.Guid }
        };


        _sut.CalculateResults(farm, viewItemsByField, fieldSystemComponent, new List<CropViewItem> { new() });
