...
public class ShelterbeltCalculatorTest
{
    ...
    /// <summary>
    ///     Gets the A value for a species that is present in Beyhan Amichev's paper: Carbon sequestration and growth of six
    ///     common tree and shrub shelterbelts in Saskatchewan, Canada
    /// </summary>
    [TestMethod]
    public void GetAReturnsCorrectValue()
    {
        var species = TreeSpecies.WhiteSpruce;
    ...
    /// <summary>
    ///     Gets the B value for a species that is present in Beyhan Amichev's paper: Carbon sequestration and growth of six
    ///     common tree and shrub shelterbelts in Saskatchewan, Canada
    /// </summary>
    [TestMethod]
    public void GetBReturnsCorrectValue()
    {
        var species = TreeSpecies.GreenAsh;
    ...
    /// <summary>
    ///     This is used for averaging: Ctree (carbon per tree), and the circumferences output from the tables ONLY. Do not
    ///     average coefficients. Do not only average final carbon. See the Algorithm Document.
    ...
    {
        var plantedTreeCountAllSpecies = new List<double>
            { 276.750, 71.375, 82.000, 144.500, 41.250, 12.375, 156.750, 301.375, 161.87 };
        var liveTreeCountAllSpecies = new List<double>
            { 42.625, 3.125, 204.750, 4.125, 89.375, 88.625, 108.000, 79.375, 184.50 };
        var result =
            calc.CalculatePercentMortalityOfALinearPlanting(plantedTreeCountAllSpecies, liveTreeCountAllSpecies);
        Assert.AreEqual(
            100.0 * (plantedTreeCountAllSpecies.Sum() - liveTreeCountAllSpecies.Sum()) /
            plantedTreeCountAllSpecies.Sum(), result, 0.0001);
    }
